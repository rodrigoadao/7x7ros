...
//custom moskaum
/**
 * Log damage dealt to an elemental
 * Similar to mob_log_damage but for elementals
 */
void elemental_log_damage(s_elemental_data* ed, block_list* src, int64 damage)
{
	uint32 char_id = 0;
	int32 flag = MDLF_NORMAL;
	// Don't log master's attacks (they already see HP)
	if( ed->master && ed->master->status.char_id == char_id ){
		return;
	}
	if( damage < 0 )
	...
	if( !damage )
		return; // No damage to log

	// Get char_id from source
	switch( src->type )
	{
		case BL_PC:
		{
	...
	if( ed->dmglog.size() == DAMAGELOG_SIZE ){
		// Remove oldest entry
		ed->dmglog.pop_front();
	}
	...
	ed->dmglog.push_back( dmg );
}
/**
 * Called when elemental receives damage
 * Similar to mob_damage but for elementals
 */
void elemental_damage(s_elemental_data *ed, struct block_list *src, int32 damage)
{
	if (src != nullptr) {
		// Log damage
		elemental_log_damage(ed, src, static_cast<int64>(damage));
	}

#if PACKETVER >= 20120404
	// Show HP bar to all characters who already hit the elemental
	for( const auto& entry : ed->dmglog ){
		...
		if( !check_distance_bl( ed, sd, AREA_SIZE ) ){
			continue;
		}

		// Send elemental HP info to this player
		clif_elemental_hp_bar( ed, sd->fd );
	}
#endif
}