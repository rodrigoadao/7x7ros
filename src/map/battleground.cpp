...
bool bg_team_warp(int32 bg_id, uint16 mapindex, int16 x, int16 y)
{
	...
	if (bgteam) {
		for (const auto &pl_sd : bgteam->members) {
			// Arena7x7: Nao teleportar tumbas (jogadores mortos)
			if (arena7x7_is_player_dead(pl_sd.sd->status.char_id)) {
#if ARENA7X7_DEBUG
				ShowInfo("Arena7x7 DEBUG: bg_team_warp pulando tumba %s (char_id: %u)\n", 
					pl_sd.sd->status.name, pl_sd.sd->status.char_id);
#endif
...
bool bg_member_respawn(map_session_data *sd)
{
	...
	// Arena7x7: Se está em partida, a morte já foi tratada em pc_dead
	// Não fazer respawn - jogador já está como tumba
	if (arena7x7_is_in_match(sd->status.char_id)) {
		return true; // Handled - não fazer respawn
	}
