...
struct unit_data *unit_bl2ud(struct block_list *bl)
{
	...
	{
	case BL_PC:
		return &((map_session_data *)bl)->ud;
	case BL_MOB:
		return &((struct mob_data *)bl)->ud;
	case BL_PET:
		return &((struct pet_data *)bl)->ud;
	case BL_NPC:
		return &((struct npc_data *)bl)->ud;
	case BL_HOM:
		return &((struct homun_data *)bl)->ud;
	case BL_MER:
		return &((s_mercenary_data *)bl)->ud;
	case BL_ELEM:
		return &((s_elemental_data *)bl)->ud;
	default:
		return nullptr;
	}
}
...
TBL_PC *unit_get_master(struct block_list *bl)
{
	if (bl)
		...
		{
		case BL_HOM:
			return (((TBL_HOM *)bl)->master);
		case BL_ELEM:
			return (((TBL_ELEM *)bl)->master);
		case BL_PET:
			return (((TBL_PET *)bl)->master);
		case BL_MER:
			return (((TBL_MER *)bl)->master);
		}
	return nullptr;
}
...
int32 *unit_get_masterteleport_timer(struct block_list *bl)
{
	if (bl)
		...
		{
		case BL_HOM:
			return &(((TBL_HOM *)bl)->masterteleport_timer);
		case BL_ELEM:
			return &(((TBL_ELEM *)bl)->masterteleport_timer);
		case BL_PET:
			return &(((TBL_PET *)bl)->masterteleport_timer);
		case BL_MER:
			return &(((TBL_MER *)bl)->masterteleport_timer);
		}
	return nullptr;
}
...
int32 unit_check_start_teleport_timer(struct block_list *sbl)
{
	...
	case BL_HOM:
		...
		break;
	case BL_ELEM:
		max_dist = MAX_ELEDISTANCE;
		break;
	case BL_PET:
		max_dist = AREA_SIZE;
		break;
	case BL_MER:
		max_dist = MAX_MER_DISTANCE;
		break;
	}
...
static TIMER_FUNC(unit_walktoxy_timer)
{
	...
	switch (bl->type)
	{ // avoid useless cast, we can only be 1 type
	case BL_PC:
		sd = BL_CAST(BL_PC, bl);
		break;
	case BL_MOB:
...
int32 unit_warp(struct block_list *bl, int16 m, int16 x, int16 y, clr_type type)
{
	...
	else if (bl->type != BL_NPC && map_getcell(m, x, y, CELL_CHKNOREACH))
	{ // Invalid target cell
		ShowWarning("unit_warp: Specified non-walkable target cell: %d (%s) at [%d,%d]\n", m, map[m].name, x, y);

		if (!map_search_freecell(nullptr, m, &x, &y, 4, 4, 1))
...
int32 unit_skilluse_id2(struct block_list *src, int32 target_id, uint16 skill_id, uint16 skill_lv, int32 casttime, int32 castcancel, bool ignore_range)
{
	...
	else if (src->type == BL_HOM)
	{
		...
		case MH_TINDER_BREAKER:
		{
			...
			if (sc->getSCE(SC_COMBO) && sc->getSCE(SC_COMBO)->val1 == skill_id2)
			{ // It's a combo
				target_id = sc->getSCE(SC_COMBO)->val2;
				combo = 1;
				casttime = -1;
	...
	if (sd)
	{
		...
		case DC_WINKCHARM:
			...
			break;
		case AB_CLEARANCE:
			// Verificar se o target está na party antes do cast começar
			if (src != target && battle_check_target(src, target, BCT_PARTY) <= 0)
			{
				clif_skill_fail(*sd, skill_id, USESKILL_FAIL_TOTARGET);
				return 0;
			}
		...
		// 	}
		// 	break;
		//início custom moskaum pra redenção não iniciar cast em alvo escondido
				case CR_DEVOTION:
			// Bloquear início de cast se o alvo estiver escondido
			// Mas permitir execução mesmo se o alvo esconder durante o cast
			{
			...
			if (target->type == BL_PC)
			{
				...
				uint8 devotions_in_range = 0;
				
				// Count devotions that are in range
				for (uint8 i = 0; i < count; i++)
				{
					if (sd->devotion[i] != 0)
					{
						block_list *devoted_bl = map_id2bl(sd->devotion[i]);
				...
				// If all slots in range are occupied, fail
		...
		// Additional pre-cast checks for skills that may fail due to map conditions
		// after passing basic requirements but before actual execution.
		// This prevents skill titles from being shown when the skill will fail later.
		switch (skill_id)
		{
		case AL_PNEUMA:
		case SC_BLOODYLUST:
		...
		{
			// These ground skills fail if all cells would be blocked by Land Protector
			// We need to check the skill layout to see if any cells will be valid
			int16 check_x = target ? target->x : src->x;
			int16 check_y = target ? target->y : src->y;

			// Get the skill unit layout to check all cells that would be affected
			struct s_skill_unit_layout *layout = skill_get_unit_layout(skill_id, skill_lv, src, check_x, check_y);
			if (layout != nullptr)
			{
				int32 valid_cells = 0;
				int32 total_cells = 0;
				...
				{
					int32 ux = check_x + layout->dx[i];
					int32 uy = check_y + layout->dy[i];

					// Check if this cell is within map bounds
					if (ux > 0 && uy > 0 && ux < map_getmapdata(src->m)->xs && uy < map_getmapdata(src->m)->ys)
					{
						total_cells++;
						// Check if this cell is not blocked by Land Protector
				...
				// If no valid cells (all blocked), the skill will fail
				if (total_cells > 0 && valid_cells == 0)
				{
					if (sd)
		...
		case HP_BASILICA:
		{
			// Basilica fails if cast on Land Protector
			int16 check_x = target ? target->x : src->x;
			int16 check_y = target ? target->y : src->y;

			if (map_getcell(src->m, check_x, check_y, CELL_CHKLANDPROTECTOR))
			{
				if (sd)
					clif_skill_fail(*sd, skill_id, USESKILL_FAIL);
				return 0;
...
case SR_TIGERCANNON:
		...
		break;
	case SR_GATEOFHELL:
		// SR_GATEOFHELL mantém cast time mesmo em combo
		combo = 1;
		break;
//
...
//fim do custom
	...
	// Pre-cast check: Dispell should fail immediately if target is hidden (prevent cast begin)
	if (skill_id == SA_DISPELL && target != nullptr && target->type == BL_PC)
	{
		status_change *tsc = status_get_sc(target);
...
int32 unit_skilluse_pos2(struct block_list *src, int16 skill_x, int16 skill_y, uint16 skill_id, uint16 skill_lv, int32 casttime, int32 castcancel, bool ignore_range)
{
	...
	// Additional pre-cast checks for ground skills that may fail due to map conditions
	// after passing basic requirements but before actual execution.
	// This prevents skill titles from being shown when the skill will fail later.
	if (sd)
	{
		...
		{
		case AL_PNEUMA:
		case MG_SAFETYWALL: // Added Safety Wall to the pre-cast check
		case SC_BLOODYLUST:
		case WZ_FIREPILLAR:
		case WM_POEMOFNETHERWORLD:
		case SO_WATER_INSIGNIA:
		...
		{
			// These ground skills fail if all cells would be blocked by Land Protector
			// Exception: MG_SAFETYWALL can be cast on Bloody Lust even if there's Land Protector
			struct s_skill_unit_layout *layout = skill_get_unit_layout(skill_id, skill_lv, src, skill_x, skill_y);
			if (layout != nullptr)
			{
				int32 valid_cells = 0;
				int32 total_cells = 0;
				...
				// For Safety Wall, check if there's Bloody Lust in the target area
				if (skill_id == MG_SAFETYWALL)
				{
					has_bloodylust = map_foreachincell([](struct block_list *bl, va_list ap) -> int
													   {
						if (bl->type == BL_SKILL) {
							struct skill_unit *su = (struct skill_unit *)bl;
							return (su->group && su->group->skill_id == SC_BLOODYLUST);
						}
						return 0; }, src->m, skill_x, skill_y, BL_SKILL);
				}

				for (int i = 0; i < layout->count; i++)
				{
					int32 ux = skill_x + layout->dx[i];
					int32 uy = skill_y + layout->dy[i];
					...
					if (ux > 0 && uy > 0 && ux < map_getmapdata(src->m)->xs && uy < map_getmapdata(src->m)->ys)
					{
						...
						// Special case: Safety Wall on Bloody Lust bypasses Land Protector check
						if (skill_id == MG_SAFETYWALL && has_bloodylust && ux == skill_x && uy == skill_y)
						{
							valid_cells++;
						}
						// Check if this cell is not blocked by Land Protector
						else if (!map_getcell(src->m, ux, uy, CELL_CHKLANDPROTECTOR))
						{
							valid_cells++;
						}
					}
		...
		{
			// Basilica fails if cast on Land Protector
			if (map_getcell(src->m, skill_x, skill_y, CELL_CHKLANDPROTECTOR))
			{
				clif_skill_fail(*sd, skill_id, USESKILL_FAIL);
				return 0;
			}
	...
	// Check if target cell is snipeable (non-walkable but can shoot through)
	// Ground skills should fail immediately on snipeable cells without consuming resources
	if (sd)
	{
		// First check if target cell is non-walkable
		if (map_getcell(src->m, skill_x, skill_y, CELL_CHKNOPASS))
		{
			// Then check if there's line of sight (can shoot through)
			// If path_search_long returns true, it means we can shoot through this wall
			// making it a "snipeable" cell where ground skills shouldn't work
			if (path_search_long(nullptr, src->m, src->x, src->y, skill_x, skill_y, CELL_CHKWALL))
			{
				// Cell is snipeable - fail the skill immediately
				// This happens before cast starts, so no SP/items consumed
				clif_skill_fail(*sd, skill_id, USESKILL_FAIL);
				return 0;
			}
		}
...
bool unit_can_attack(struct block_list *bl, int32 target_id)
{
	...
	if (bl->type == BL_PC)
	{
		...
		// Arena7x7: Jogador morto (tumba) n�o pode atacar
		if (sd && arena7x7_is_player_dead(sd->status.char_id))
			return false;
	}
...
int32 unit_remove_map_(struct block_list *bl, clr_type clrtype, const char *file, int32 line, const char *func)
{
	...
	case BL_PC:
	{
		...
		if (sd->shadowform_id)
		{ // If shadow target has leave the map
