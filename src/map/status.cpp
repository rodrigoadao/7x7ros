...
int32 status_damage(struct block_list *src, struct block_list *target, int64 dhp, int64 dsp, int64 dap, t_tick walkdelay, int32 flag, uint16 skill_id)
{
	...
	if (!(flag & 1))
	{
		...
		//custom moskaum pra redentado n�o ter cast interrompido
		if (target->type == BL_PC)
			pc_bonus_script_clear(BL_CAST(BL_PC, target), BSF_REM_ON_DAMAGED);
		
		// Don't cancel cast if target has DEVOTION active and within range
		bool skip_cast_cancel = false;
		if (sc && sc->getSCE(SC_DEVOTION) && sc->getSCE(SC_DEVOTION)->val1)
		{
			block_list *d_bl = map_id2bl(sc->getSCE(SC_DEVOTION)->val1);
			if (d_bl && check_distance_bl(target, d_bl, sc->getSCE(SC_DEVOTION)->val3))
				skip_cast_cancel = true;
		}
	...
	// Arena7x7: Registrar dano entre jogadores
	if (src && target && hp > 0)
		arena7x7_on_damage(src, target, skill_id, hp, false);
	...
	if (target->type == BL_PC)
	{
		...
		// Arena7x7: Interceptar morte ANTES de disparar die_event
		// Se jogador est� em partida Arena7x7, transform�-lo em t�mulo e N�O disparar die_event
		if (sd->bg_id && arena7x7_is_in_match(sd->status.char_id))
		{
			// Verificar se j� est� morto (tumba) - evita processar duas vezes
			if (!arena7x7_is_player_dead(sd->status.char_id))
			{
#if ARENA7X7_DEBUG
				ShowInfo("Arena7x7 DEBUG: status_damage - %s morreu em partida Arena7x7 (skill_id=%u)\n", sd->status.name, skill_id);
#endif
				// Obter o killer (se for player)
				map_session_data *killer_sd = nullptr;
				...
				else if (src)
				{
					...
					{
						killer_sd = BL_CAST(BL_PC, master);
					}
				}
				
				// Registrar morte e transformar em t�mulo
				// arena7x7_on_death vai disparar o die_event manualmente ap�s transformar
				arena7x7_on_death(killer_sd, sd, skill_id);
			}
			// N�o disparar die_event aqui - j� foi disparado em arena7x7_on_death
			// Pular para npc_script_event
		}
		else if (sd->bg_id)
		{
...
int32 status_revive(struct block_list *bl, unsigned char per_hp, unsigned char per_sp, unsigned char per_ap)
{
	...
	// Arena7x7: Bloquear ressurrei��o de jogadores mortos na partida
	if (bl->type == BL_PC) {
		map_session_data* sd = (map_session_data*)bl;
		if (arena7x7_is_player_dead(sd->status.char_id)) {
			ShowInfo("Arena7x7: Bloqueando ressurreicao de %s (morto na partida)\n", sd->status.name);
			return 0; // Bloquear ressurrei��o
		}
	}
...
bool status_check_skilluse(struct block_list *src, struct block_list *target, uint16 skill_id, int32 flag)
{
	...
	// Allow HT_REMOVETRAP and MA_REMOVETRAP to target any skill unit (trap)
	if ((skill_id == HT_REMOVETRAP || skill_id == MA_REMOVETRAP) && target->type == BL_SKILL)
	{
		return true;
	}
	...
	if (tsc != nullptr && !tsc->empty())
	{
		...
		if ((skill_id == PR_LEXAETERNA || skill_id == AB_VITUPERATUM) && (tsc->getSCE(SC_FREEZE) || tsc->getSCE(SC_STONE)))
			return false;
	...
	// Special case: Arrow Storm should be allowed to target even hidden targets
	// since it will skip direct damage but still apply splash damage
	if (skill_id == RA_ARROWSTORM || skill_id == NPC_ARROWSTORM)
		hide_flag = 0;

	// Note: SA_DISPELL targeting check is handled in skill_castend_id_check() to allow targeting but fail during execution
	...
	case BL_PC:
	{
		...
		if (tsc)
		{
			...
			{
				// Exce��o: SR_CURSEDCIRCLE pode detectar alvos com SC__FEINTBOMB
				if (skill_id == SR_CURSEDCIRCLE || skill_id == NPC_SR_CURSEDCIRCLE)
					return true; // Campo Amaldi�oado sempre detecta Shadow Chaser invis�vel
...
static uint32 status_calc_maxhp_pc(map_session_data &sd, uint32 vit)
{
	...
	// Aplica bonus fixos (incluindo SC_PROMOTE_HEALTH_RESERCH, SC_ANGELUS, etc)
	dmax += status_get_hpbonus(&sd, STATUS_BONUS_FIX);

	// ? SALVAR HP BASE para c�lculo de percentuais
	double base_hp = dmax; // HP base completo com todos os bonus fixos

	// Calcular bonus percentuais de equipamentos sobre HP base
	double equip_bonus = (base_hp * status_get_hpbonus_equip(&sd) / 100);
	double item_bonus = (base_hp * status_get_hpbonus_item(&sd) / 100);

	// Calcular bonus percentuais de buffs sobre HP base (mesma base!)
	double rate_bonus = static_cast<int64>(base_hp * status_get_hpbonus(&sd, STATUS_BONUS_RATE) / 100);

	// Somar tudo
	dmax += equip_bonus + item_bonus + rate_bonus;
...
// 	//custom pra corrigir calculo de hp, hierarquia de primeiro flat depois %, sendo que porcentagem de equip e buff devem ser somadas antes de multiplicarem o dmax
// 	// NOVA ORDEM: Aplica buffs PERCENTUAIS PRIMEIRO (sobre HP base)
...
// 	//custom moskaum pra ajustar calculo hp
// 	double equip_bonus = (dmax * status_get_hpbonus_equip(&sd) / 100);
// 	//fim de custom
...
// 	//custom moskaum pra ajustar calculo hp
// 	dmax += static_cast<int64>(base_hp_for_percent_bonuses * status_get_hpbonus(&sd, STATUS_BONUS_RATE) / 100);
// 	//fim do custom
...
// 	return cap_value(static_cast<uint32>(dmax), 1, std::numeric_limits<uint32>::max());
// }
// fim do orignal mais ou menos editado por�m bem comentado
...
static uint32 status_calc_maxsp_pc(map_session_data &sd, uint32 int_)
{
    ...
    // ETAPA 1: Aplicar b�nus de INT e truncar
    if (int_ > 0)
    {
        dmax *= (1.0 + int_ * 0.01);
        dmax = static_cast<int32>(dmax);  // ? TRUNCA
    }

    // ETAPA 2: Aplicar multiplicador transcendente e truncar
    if (sd.class_ & JOBL_UPPER)
    {
        dmax *= 1.25;
        dmax = static_cast<int32>(dmax);  // ? TRUNCA
    ...
    // Save base SP for percentage calculations
    double base_sp = dmax;

    double equip_bonus = (base_sp * status_get_spbonus_equip(&sd) / 100);
    double item_bonus = (base_sp * status_get_spbonus_item(&sd) / 100);

    double rate_bonus = static_cast<int64>(base_sp * status_get_spbonus(&sd, STATUS_BONUS_RATE) / 100);
...
//calculo antigo
// in�cio do custom pra arrumar hierarquia de calculo sp
// static uint32 status_calc_maxsp_pc(map_session_data &sd, uint32 int_)
// {
// 	std::shared_ptr<s_job_info> job = job_db.find(sd.status.class_);
...
// 	//custom moskaum pra consertar calculo sp
// 	// Save base SP before applying percentage bonuses
// 	double base_sp_for_percent_bonuses = dmax;
// 	double equip_bonus = (base_sp_for_percent_bonuses * status_get_spbonus_equip(&sd) / 100);
// 	//fim do custom
...
// 	//custom moskaum pra consertar calculo sp
// 	double item_bonus = (base_sp_for_percent_bonuses * status_get_spbonus_item(&sd) / 100);
// 	//fim do custom
...
// 	//custom moskaum pra consertar calculo sp
// 	dmax += static_cast<int64>(base_sp_for_percent_bonuses * status_get_spbonus(&sd, STATUS_BONUS_RATE) / 100);
// 	//fim do custom
...
int32 status_calc_pc_sub(map_session_data *sd, uint8 opt)
{
	...
	for (i = 0; i < EQI_MAX; i++)
	{
		...
		if (opt & SCO_FIRST && sd->inventory_data[index]->equip_script && (pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(sd->inventory_data[index], sd->m)))
	...
	for (i = 0; i < EQI_MAX; i++)
	{
		...
		if (sd->inventory_data[index])
		{
			...
			{ // Uses MAX_SLOTS to support Soul Bound system [Inkfish]
				...
				if (!data)
					continue;
				if (opt & SCO_FIRST && data->equip_script && (pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(data.get(), sd->m)))
				{ // Execute equip-script on login
...
// 	}
//fim do original
//custom pra consertar arredondamento de aspd
	if (flag[SCB_ASPD])
	{
		int32 amotion;
		...
		if (bl.type == BL_HOM)
		{
			...
			amotion = status_calc_aspd_rate(&bl, sc, amotion);
			// CORRE��O: Round correto em vez de truncamento
			amotion = (amotion * status->aspd_rate + 500) / 1000;
#endif
		...
		else if (bl.type == BL_PC)
		{
			...
			if (sd->ud.skilltimer != INVALID_TIMER && (skill_lv = pc_checkskill(sd, SA_FREECAST)) > 0)
#ifdef RENEWAL_ASPD
				// CORRE��O: Round correto para Freecast
				amotion = (amotion * 5 * (skill_lv + 10) + 50) / 100;
#else
				amotion += (2000 - amotion) * (55 - 5 * (skill_lv + 1)) / 100; // Increases amotion to reduce ASPD to the corresponding absolute percentage for each level (overriding other adjustments)
#endif
...
#ifdef RENEWAL_ASPD
			// RE ASPD % modifier
			// CORRE��O CR�TICA: Round correto - RESOLVE O ERRO DE -2 ASPD!
			amotion += (max(195 - amotion, 2) * (status->aspd_rate2 + status_calc_aspd(&bl, sc, false)) + 50) / 100;
		...
		{ // Mercenary and mobs
			...
			// For now we solve it by making sure we never increase adelay through aspd_rate and then cap it to amotion
			// CORRE��O: Round correto em vez de truncamento
			temp = (b_status->adelay * min(status->aspd_rate, 1000) + 500) / 1000;
			status->adelay = cap_value(temp, AMOTION_DIVIDER_NOPC * status->amotion, MIN_ASPD);
		}
	}
//fim de custom
...
static defType status_calc_def(struct block_list *bl, status_change *sc, int32 def)
{
	...
	//custom moskaum pra echo song ser percentual e n�o flat
	if (sc->getSCE(SC_ECHOSONG))
		def += def * sc->getSCE(SC_ECHOSONG)->val3 / 100;
	//
	...
	if (sc->getSCE(SC_MARSHOFABYSS))
		def -= def * sc->getSCE(SC_MARSHOFABYSS)->val4 / 100;
...
static int16 status_calc_def2(struct block_list *bl, status_change *sc, int32 def2)
{
	...
	if (sc->getSCE(SC_ANGELUS))
		...
		//custom moskaum pra angelus arredondar soft def
		def2 += (status_get_vit(bl) / 2 * sc->getSCE(SC_ANGELUS)->val2 + 50) / 100;
		//fim do custom
#else
	...
	if (sc->getSCE(SC_MARSHOFABYSS))
		def2 -= def2 * sc->getSCE(SC_MARSHOFABYSS)->val4 / 100;
...
static defType status_calc_mdef(struct block_list *bl, status_change *sc, int32 mdef)
{
	...
	if (sc->getSCE(SC_MARSHOFABYSS))
		mdef -= mdef * sc->getSCE(SC_MARSHOFABYSS)->val4 / 100;
	if (sc->getSCE(SC_SOULGOLEM))
...
static int16 status_calc_mdef2(struct block_list *bl, status_change *sc, int32 mdef2)
{
	...
	if (sc->getSCE(SC_MARSHOFABYSS))
		mdef2 -= mdef2 * sc->getSCE(SC_MARSHOFABYSS)->val4 / 100;
...
static uint16 status_calc_speed(struct block_list *bl, status_change *sc, int32 speed)
{
	...
	if (sc->getSCE(SC_STEELBODY))
		speed = 200;
	// vers�o cremane
	//  if (sc->getSCE(SC_DEFENDER) && sc->getSCE(SC_DEFENDER)->val3 == 0) // [MODIFICADO] Verifica se val3 � 0 (slow ativo)
	//  	speed = max(speed, 200);
	// vers�o moskaum
	if (sc->getSCE(SC_DEFENDER) && sc->getSCE(SC_DEFENDER)->val3 == 0 && !sc->getSCE(SC_DEVOTION))
		speed = max(speed, 200);
	// fim de custom
...
int32 status_get_class(struct block_list *bl)
{
	...
	case BL_MOB:
		return ((TBL_MOB *)bl)->vd->look[LOOK_BASE]; // Class used on all code should be the view class of the mob.
	case BL_PET:
...
struct status_data *status_get_base_status(struct block_list *bl)
{
	...
	case BL_NPC:
		return ((mobdb_checkid(((TBL_NPC *)bl)->class_) == 0) ? &((TBL_NPC *)bl)->status : nullptr);
	default:
		return nullptr;
	}
}
...
static bool status_change_start_post_delay(block_list *src, block_list *bl, sc_type type, int32 val1, int32 val2, int32 val3, int32 val4, int32 tick, uint8 flag)
{
	...
	if (sc->opt1 > OPT1_NONE && scdb->opt1 > OPT1_NONE)
	{
		for (const auto &status_it : status_db)
		{
			...
			// Allow Freeze and Deepsleep to coexist
	...
	if (!(flag & SCSTART_LOADED)) // &4 - Do not parse val settings when loading SCs
		...
		case SC_AUTOGUARD:
			...
			// Propagate to devotees even when SCSTART_NOAVOID is set (from devotion inheritance)
			else if ((flag & SCSTART_NOAVOID) && (bl->type & (BL_PC | BL_MER)))
			{
				if (sd)
				{
				...
				{
					map_session_data *tsd = ((TBL_MER *)bl)->master;
					if (tsd)
						status_change_start(src, tsd, type, 10000, val1, val2, 0, 0, tick, SCSTART_NOAVOID | SCSTART_NOICON);
				}
			}
		...
		case SC_DEFENDER:
			...
			{
				// vers�o cremane
				// val2 = 15 + 16 * val1;	// Damage reduction: 15% base + 80% skill at max level (15 + 16*5 = 95)
				// vers�o moskaum
				val2 = 0 + 16 * val1;	// pq n quero q tenha os 15 base
				val3 = 0;				// Unused, previously speed adjustment
		...
		case SC_REGENERATION:
			...
			break;
			// a seguir, vers�o cremane conforme latam, que vai ser suprimida pq quero vers�o sakray
			//  case SC_DEVOTION:
			//  {
			...
			// 	status_change *d_sc = status_get_sc(d_bl); // SC do Caster (Paladino)
			// 	status_change *t_sc = sc;				   // SC do Target (Alvo)

			// 	// Verifica se o alvo j� est� sob efeito de SC_DEVOTION
			// 	bool is_refresh = (t_sc && t_sc->getSCE(SC_DEVOTION) != nullptr);

			// 	if (d_sc != nullptr)
			// 	{
			// 		// Inherits status from source
			// 		const enum sc_type types[] = {SC_AUTOGUARD, SC_DEFENDER, SC_REFLECTSHIELD, SC_ENDURE};
			// 		// Processar todos os buffs exceto SC_ENDURE em mapas GvG/BG
			// 		int32 max_buffs = (map_flag_gvg2(bl->m) || map_getmapflag(bl->m, MF_BATTLEGROUND)) ? 3 : 4;

			// 		for (int32 i = 0; i < max_buffs; i++)
			// 		{
			// 			enum sc_type type2 = types[i];
			// 			status_change_entry *d_sce = d_sc->getSCE(type2); // Buff no Caster

			// 			if (d_sce != nullptr)
			// 			{
			// 				// Caster tem o buff - transferir para o alvo
			// 				if (type2 == SC_DEFENDER && is_refresh && t_sc->getSCE(SC_DEFENDER))
			// 				{
			// 					// � um refresh do SC_DEFENDER - aplicar debuff (sem valor base)
			// 					status_change_entry *t_sce_def = t_sc->getSCE(SC_DEFENDER);

			// 					// Renova��o: apenas o valor da skill (16 * skill_lv) sem base
			// 					t_sce_def->val2 = 16 * d_sce->val1; // 80% para skill n�vel 5

			// 					// Define val3 = 1 para sinalizar "sem slow"
			// 					t_sce_def->val3 = 1;

			// 					// Recalcula a velocidade e aspd do alvo imediatamente
			// 					std::bitset<SCB_MAX> calc_flag;
			// 					calc_flag.set(SCB_ASPD);
			// 					calc_flag.set(SCB_SPEED);
			// 					status_calc_bl_(bl, calc_flag);
			// 				}
			// 				else if (type2 == SC_DEFENDER)
			// 				{
			// 					// Primeira aplica��o do Defender - valores completos
			// 					status_change_start(d_bl, bl, type2, 10000, d_sce->val1, d_sce->val2, 0, 0, skill_get_time(status_db.getSkill(type2), d_sce->val1), SCSTART_NOAVOID);
			// 				}
			// 				else
			// 				{
			// 					// Para outros buffs (AutoGuard, ReflectShield, Endure)
			// 					// Preservar val2 para AutoGuard (cont�m c�lculos de chance)
			// 					int32 val2_transfer = (type2 == SC_AUTOGUARD) ? d_sce->val2 : 0;
			// 					status_change_start(d_bl, bl, type2, 10000, d_sce->val1, val2_transfer, 0, (type2 == SC_REFLECTSHIELD ? 1 : 0), skill_get_time(status_db.getSkill(type2), d_sce->val1), SCSTART_NOAVOID | SCSTART_NOICON);
			// 				}
			// 			}
			// 		}
			// 	}
			...
			if (d_sc != nullptr)
			{
				...
				for (int32 i = 0; i < max_buffs; i++)
				{
					...
					if (d_sce != nullptr)
					{
						...
						{
							// Aplicar Defender sem redu��o de velocidade (val3 = 1)
							status_change_start(d_bl, bl, type2, 10000, d_sce->val1, d_sce->val2, 1, 0, skill_get_time(status_db.getSkill(type2), d_sce->val1), SCSTART_NOAVOID);
						}
						else
						{
							// Para outros buffs (AutoGuard, ReflectShield, Endure)
		...
		case SC_MARSHOFABYSS:
			if (bl->type == BL_PC)
				...
				val2 = 6 * val1; // AGI and DEX Reduction
			val3 = 10 * val1;	 // Movement Speed Reduction
			// Redu��o DEF/MDEF = {10 + [(Nv. de classe � 2) + (INT do usu�rio � 10)]}%
			if (src) {
				status_data *sstatus = status_get_status_data(*src);
				map_session_data *ssd = BL_CAST(BL_PC, src);
				int32 job_level = ssd ? ssd->status.job_level : 50; // Default 50 para monstros
				val4 = 10 + (job_level / 2) + (sstatus->int_ / 10);
			} else {
				val4 = 10; // Valor m�nimo se n�o houver source
			}
			break;
		//
		case SC_FREEZE_SP:
		...
		case SC__WEAKNESS:
			...
			// Unequip weapon and shield BEFORE applying status
			if (sd)
			{
				int16 i;
				// Unequip weapons (left and right hand)
				i = sd->equip_index[EQI_HAND_L];
				if (i >= 0 && sd->inventory_data[i] && sd->inventory_data[i]->type == IT_WEAPON)
					pc_unequipitem(sd, i, 3);
				...
				// Unequip shield (left hand armor)
				i = sd->equip_index[EQI_HAND_L];
				if (i >= 0 && sd->inventory_data[i] && sd->inventory_data[i]->type == IT_ARMOR)
					pc_unequipitem(sd, i, 3);
			}
		...
		case SC_RUSHWINDMILL:	 // ATK Increase
			...
			break;
		case SC_ECHOSONG:
			//ShowInfo("ECHOSONG DEBUG ANTES: val1=%d, val2=%d, val3=%d\n", val1, val2, val3);
			//custom moskaum
			val3 = 6 * val1 + val2 + val3 / 4; // val3 agora contem job_level, DEF Increase em %
			//ShowInfo("ECHOSONG DEBUG DEPOIS: val3_final=%d\n", val3);
			//fim custom
			//original
		...
		case SC_WIND_STEP_OPTION:
			val2 = 52.5; // % Increase speed and flee.
			break;
		...
		// 	break;
		//fim original
		//custom moskaum pra fazer o buff movspeed da vayu durar
		case SC_CIRCLE_OF_FIRE:
		case SC_FIRE_CLOAK:
		case SC_WATER_DROP:
		case SC_WATER_SCREEN:
		...
		case SC_SOLID_SKIN:
			...
			tick_time = val3;
			break;
		case SC_WIND_STEP:              // ? Separar SC_WIND_STEP
			val2 = 1;                    // ? 1 SP por tick
			val3 = 5000;                 // ? A cada 5 segundos
			tick_time = val3;
			break;
		//fim do custom
	...
	if (!(flag & SCSTART_NOICON) && !(flag & SCSTART_LOADED && scdb->flag[SCF_DISPLAYPC] || scdb->flag[SCF_DISPLAYNPC]))
	{
		...
		// Special case: Don't send SHADOWFORM visual effect if player has FEINTBOMB invisibility active
		// The visual effect will be restored when FEINTBOMB ends naturally
		bool skip_visual = (type == SC__SHADOWFORM && sc && sc->getSCE(SC__FEINTBOMB));
...
int32 status_change_end(struct block_list *bl, enum sc_type type, int32 tid)
{
	...
	case SC_AUTOGUARD:
	{
		...
		{ // Clear Status from others
			int32 i;
			// Special handling for SC_DEFENDER to prevent title display
			if (type == SC_DEFENDER && sd)
			{
				// Simply end the status without displaying title
				// The title will not be shown because we're ending it here
			}

			// Update devotion targets when paladin loses/gains buffs
			for (i = 0; i < MAX_DEVOTION; i++)
			{
	...
	case SC_DANCING:
	{
		...
		if (val4 && val4 != BCT_SELF && (dsd = map_id2sd(val4)))
...
case SC_WHITEIMPRISON:
	...
	case SC__FEINTBOMB:
	{
		// When FEINTBOMB ends, restore SHADOWFORM visual effect if still active
		if (sc && sc->getSCE(SC__SHADOWFORM))
		{
			// Calculate remaining time for SHADOWFORM (val4 contains remaining seconds)
			t_tick remaining_time = sc->getSCE(SC__SHADOWFORM)->val4 * 1000; // Convert seconds to milliseconds

			// Refresh SHADOWFORM visual effect with correct remaining time
			clif_status_change(bl, EFST_SHADOWFORM, 1, remaining_time, sc->getSCE(SC__SHADOWFORM)->val1, sc->getSCE(SC__SHADOWFORM)->val2, sc->getSCE(SC__SHADOWFORM)->val3);

			// Update visual options since FEINTBOMB invisibility is ending
			clif_changeoption(bl);
		}

		// Critical Fix: Force visual resync for nearby players after invisibility ends
		// This ensures that visual effects are properly updated for other clients
		if (bl->type == BL_PC)
		{
			map_session_data *sd = BL_CAST(BL_PC, bl);
			if (sd)
			{
				// Add a brief timer to force visual resync after invisibility state clears
				add_timer(gettick() + 100, [](int tid, t_tick tick, int id, intptr_t data) -> int
						  {
					struct block_list *bl = map_id2bl(id);
					if (bl && bl->type == BL_PC) {
						// Force complete visual resync by clearing then spawning
		...
		// Fix: Verificar se ainda h� status de campos ativos quando FEINTBOMB acaba
		// Se o jogador saiu da �rea enquanto invis�vel, remover os status
		if (sc && bl->type == BL_PC)
		{
			bool in_neutralbarrier = false;
			bool in_stealthfield = false;
			bool in_bloodylust = false;

			// Verificar se realmente est� em �rea de campo protetor/camuflagem
			map_foreachinallrange([](struct block_list *bl, va_list ap) -> int
								  {
				struct skill_unit *unit = (struct skill_unit *)bl;
				bool *in_neutral = va_arg(ap, bool*);
				bool *in_stealth = va_arg(ap, bool*);
				bool *in_bloodylust = va_arg(ap, bool*);
				struct block_list *target = va_arg(ap, struct block_list*);
				
				if (unit->group && unit->alive && 
					unit->x == target->x && unit->y == target->y)
				{
					if (unit->group->skill_id == NC_NEUTRALBARRIER)
						*in_neutral = true;
					if (unit->group->skill_id == NC_STEALTHFIELD)
						*in_stealth = true;
					if (unit->group->skill_id == SC_BLOODYLUST)
						*in_bloodylust = true;
				}
				return 0; }, bl, 0, BL_SKILL, &in_neutralbarrier, &in_stealthfield, &in_bloodylust, bl);

			// Se n�o est� mais em campo protetor mas tem o status, remover
			if (!in_neutralbarrier && sc->getSCE(SC_NEUTRALBARRIER))
			{
				status_change_end(bl, SC_NEUTRALBARRIER);
				clif_status_change(bl, status_db.getIcon(SC_NEUTRALBARRIER), 0, 0, 0, 0, 0);
			}

			// Se n�o est� mais em campo de camuflagem mas tem o status, remover
			if (!in_stealthfield && sc->getSCE(SC_STEALTHFIELD))
			{
				status_change_end(bl, SC_STEALTHFIELD);
				clif_status_change(bl, status_db.getIcon(SC_STEALTHFIELD), 0, 0, 0, 0, 0);
			}

			// Note: BLOODYLUST handling is now done properly through skill_unit_onout
			// when player exits the area, even while invisible with FEINTBOMB
		}

		// Critical fix: Force complete visual sync for all nearby players
		// This ensures that visual effects are properly updated when invisibility ends
		if (bl->type == BL_PC)
		{
			map_session_data *sd = BL_CAST(BL_PC, bl);
			if (sd)
			{
				// Use a small delay timer to ensure proper visual synchronization
				add_timer(gettick() + 100, [](int tid, t_tick tick, int id, intptr_t data) -> int
						  {
					struct block_list *bl = map_id2bl(id);
					if (bl && bl->type == BL_PC)
					{
						// Clear the player from everyone's view and respawn
						clif_clearunit_area(*bl, CLR_OUTSIGHT);
						// Small delay before respawn to ensure client processes the clear
						add_timer(gettick() + 50, [](int tid, t_tick tick, int id, intptr_t data) -> int
						{
							struct block_list *bl = map_id2bl(id);
							if (bl) {
								clif_spawn(bl);
...
TIMER_FUNC(status_change_timer)
{
	...
	case SC_INSPIRATION:
		if (--(sce->val4) >= 0)
		{
			// Check if player has enough SP before charging
			int32 sp_cost = status->max_sp * (45 - 5 * sce->val1) / 1000;

			// If not enough SP, end inspiration immediately instead of waiting for next tick
			if (status->sp < sp_cost)
			{
				// Force immediate removal of inspiration when SP is insufficient
				break;
			}

			if (!status_charge(bl, status->max_hp * (35 - 5 * sce->val1) / 1000, sp_cost))
				break;
	...
	case SC_VACUUM_EXTREME:
		if (sce->val4 > 0)
		{
			...
			if (unit_bl2ud(bl)->walktimer == INVALID_TIMER)
			{
				...
				// Only pull/move targets in non-GvG/BG maps
				if (distance_xy(x, y, bl->x, bl->y) <= skill_get_unit_range(SO_VACUUM_EXTREME, sce->val1) &&
...
int32 status_change_timer_sub(struct block_list *bl, va_list ap)
{
	...
	case SC_CONCENTRATE:
		...
		//
		// minha altera��o
		if (tsc && tsc->getSCE(SC__SHADOWFORM) &&
	...
	case SC_RUWACH: // Reveal hidden target and deal little dammages if enemy
		...
		// custom moskaum, o comentado � o original
		//  if (tsc && tsc->getSCE(SC__SHADOWFORM) && (sce && sce->val4 > 0 && sce->val4 % 2000 == 0) && // For every 2 seconds do the checking
		//  	rnd() % 100 < 100 - tsc->getSCE(SC__SHADOWFORM)->val1 * 10)
		if (tsc && tsc->getSCE(SC__SHADOWFORM) &&
			rnd() % 100 < 100 - tsc->getSCE(SC__SHADOWFORM)->val1 * 10)
