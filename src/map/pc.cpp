...
uint8 pc_isequip(map_session_data *sd,int32 n)
{
	...
	if (!sd->sc.empty()) {
		...
		// 	return ITEM_EQUIP_ACK_FAIL;
		//
		if (sd->sc.getSCE(SC__WEAKNESS)) {
			if (item->equip & EQP_ARMS && item->type == IT_WEAPON)
				return ITEM_EQUIP_ACK_FAIL;
			if (item->equip & EQP_SHIELD && item->type == IT_ARMOR)
				return ITEM_EQUIP_ACK_FAIL;
		}
		//
	...
	//inicio custom moskaum
	// Verifica se está tentando equipar item proibido em mapa GvG/BG
	struct map_data *mapdata = map_getmapdata(sd->m);
	if (mapdata && (mapdata->getMapFlag(MF_GVG) || 
	                mapdata->getMapFlag(MF_GVG_CASTLE) || 
	                mapdata->getMapFlag(MF_BATTLEGROUND))) {
		// Lista de IDs dos itens RWC/RTC proibidos
		static const int prohibited_items[] = {
			5451, 5452, 5453, 18509, 18625, 18650, 18779, 18780, 18781, 18796,
			18828, 18829, 18830, 18831, 19949, 5892, 5893, 5894, 18686, 5603,
			5604, 5605, 18515, 18516, 18517, 19945, 19897
		};
		
		t_itemid nameid = sd->inventory.u.items_inventory[n].nameid;
		
		for (int i = 0; i < ARRAYLENGTH(prohibited_items); i++) {
			if (nameid == prohibited_items[i]) {
				clif_displaymessage(sd->fd, "Este item nao pode ser equipado em mapas GvG/Battleground.");
				return ITEM_EQUIP_ACK_FAIL;
			}
		}
	}
...
//custom moskaum pra desequipar item ao entrar em mapflag gvg ou battleground
/**
 * Check and unequip prohibited items in GvG/BG maps
 * @param sd: Player data
 */
static void pc_check_prohibited_items(map_session_data* sd, int16 m) {
	// Lista de IDs dos itens RWC/RTC proibidos em GvG/BG
	static const int prohibited_items[] = {
		5451, 5452, 5453, 18509, 18625, 18650, 18779, 18780, 18781, 18796,
		18828, 18829, 18830, 18831, 19949, 5892, 5893, 5894, 18686, 5603,
		5604, 5605, 18515, 18516, 18517, 19945, 19897
	...
	// Verifica se está em mapa GvG ou Battleground
	if (!mapdata || (!mapdata->getMapFlag(MF_GVG) && 
	                 !mapdata->getMapFlag(MF_GVG_CASTLE) && 
	                 !mapdata->getMapFlag(MF_BATTLEGROUND)))
		return;
	...
	for (int i = 0; i < EQI_MAX; i++) {
		...
		// Verifica se o item está na lista proibida
		for (int j = 0; j < ARRAYLENGTH(prohibited_items); j++) {
			if (nameid == prohibited_items[j]) {
				// Desequipa o item (2 = força o unequip)
				pc_unequipitem(sd, idx, 2);
				
				// Envia mensagem ao jogador
				char output[CHAT_SIZE_MAX];
				safesnprintf(output, sizeof(output), 
					"O item %s foi automaticamente desequipado neste mapa.", 
					itemdb_ename(nameid));
...
char pc_delitem(map_session_data *sd,int32 n,int32 amount,int32 type, int16 reason, e_log_pick_type log_type)
{
	...
	// Arena 7x7: Rastrear itens consumidos durante a partida
	if (log_type == LOG_TYPE_CONSUME && sd->inventory_data[n] != nullptr) {
		t_itemid item_id = sd->inventory.u.items_inventory[n].nameid;
		const char* item_name = sd->inventory_data[n]->name.c_str();
		arena7x7_track_item_consume(sd, item_id, item_name, amount);
	}
...
int32 pc_useitem(map_session_data *sd,int32 n)
{
	...
	// Arena7x7: Bloquear uso de itens se o jogador estiver morto (tumba) na arena
	if (arena7x7_is_player_dead(sd->status.char_id)) {
		clif_displaymessage(sd->fd, "[Arena 7x7] Voce esta eliminado e nao pode usar itens.");
		return 0;
	}
...
enum e_setpos pc_setpos(map_session_data* sd, uint16 mapindex, int32 x, int32 y, clr_type clrtype)
{
	...
	// Arena7x7: Ao fazer warp, restaurar jogador se estiver como t�mulo e remover da partida
	// DEVE ser feito ANTES de qualquer outra coisa para garantir que o disguise seja removido
	if (arena7x7_is_in_match(sd->status.char_id)) {
		// Restaurar do t�mulo (remove disguise, restaura HP/SP)
		if (arena7x7_is_player_dead(sd->status.char_id)) {
			arena7x7_restore_from_tombstone(sd);
		}
		// Remover da partida (n�o � desertor se a partida j� terminou)
		auto match = arena7x7_get_player_match(sd->status.char_id);
		bool is_deserter = (match && match->status == ARENA7X7_MATCH_ACTIVE);
		arena7x7_remove_player(sd, is_deserter);
	}
	...
	//inicio custom moskaum
	// Auto-unequip prohibited items in GvG/BG maps
	pc_check_prohibited_items(sd, m);
	//fim custom
...
//
	...
	if (src && src->type == BL_PC) {
		...
		// Arena7x7: A morte � tratada em status_damage() que tem acesso ao skill_id
		// N�o chamar arena7x7_on_death aqui para evitar duplica��o
	...
	if( mapdata_flag_gvg2(mapdata) ) {
		...
		if (bg) {
			if (bg->cemetery.map > 0) { // Respawn by BG
				// Arena7x7: Verificar se est� em partida - n�o fazer respawn autom�tico
				if (arena7x7_is_in_match(sd->status.char_id)) {
					...
					// NOTA: A morte da Arena7x7 � tratada em status_damage() que tem acesso ao skill_id
					// Este � um fallback caso status_damage n�o tenha processado (n�o deveria acontecer)
#if ARENA7X7_DEBUG
					ShowInfo("Arena7x7 DEBUG: pc_dead detectou jogador %s em partida Arena7x7 - fallback sem skill_id\n", sd->status.name);
#endif
					
					// Registrar morte no sistema Arena7x7 (fallback com skill_id=0)
					arena7x7_on_death(nullptr, sd, 0);
					
					// IMPORTANTE: Retornar 0 para indicar que a morte foi "cancelada"
					return 0;
				}
