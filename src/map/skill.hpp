...
struct s_skill_db
{
	...
	int32 custom_cast_time[MAX_SKILL_LEVEL];	   ///< Custom cast time for skills on success (per level)
	int32 custom_aftercast_delay[MAX_SKILL_LEVEL]; ///< Custom after cast delay for skills on success (per level)
...
int32 skill_custom_aftercast_delayfix(block_list *bl, uint16 skill_id, uint16 skill_lv);
...
extern bool sideslide_movement_active;		 // Flag to track NC_F_SIDESLIDE/NC_B_SIDESLIDE movement
extern bool neutralbarrier_needs_reposition; // Flag to indicate if barrier needs repositioning after sideslide
