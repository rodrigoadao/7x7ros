 ...
 * =============================================================================
 *
 * Este mùdulo implementa um sistema de arena PvP onde duas equipes de atù 7
 * jogadores cada se enfrentam. O sistema inclui:
 *
 * - Gerenciamento de partidas (criar, iniciar, finalizar, cancelar)
 * - Tracking detalhado de estatùsticas (dano, kills, assists, healing)
 * - Sistema de morte permanente (jogador vira tumba atù o fim da partida)
 * - Persistùncia em banco de dados SQL
 * - Integraùùo com sistema de ranking e seasons
 * - Logs detalhados para anùlise no site
 *
 * Configuraùùo:
 * - Mùximo de jogadores por time: definido em ARENA7X7_MAX_PLAYERS_PER_TEAM
 * - Janela de assistùncia: ARENA7X7_ASSIST_WINDOW (5 segundos)
 *
 * =============================================================================
 */
...
/// Usa shared_ptr para gerenciamento automùtico de memùria
std::unordered_map<uint32, std::shared_ptr<s_arena7x7_match>> arena7x7_matches;

/// Mapa rùpido para encontrar qual partida um jogador estù
/// Formato: char_id -> match_id
std::unordered_map<uint32, uint32> arena7x7_player_match;

/// Contador incremental para gerar IDs ùnicos de partidas
...
uint32 arena7x7_create_match(const char *map_name)
{
	...
	// Incrementar contador (serù inicializado do SQL no init)
...
bool arena7x7_add_player(uint32 match_id, map_session_data *sd, e_arena7x7_team team, bool is_leader)
{
	...
	// Guardar referùncia ao shared_ptr para evitar problemas se o iterador for invalidado
	std::shared_ptr<s_arena7x7_match> match = it->second;

	// Validar se o ponteiro do match ù vùlido (verificar ponteiro interno tambùm)
	if (!match || !match.get())
	{
	...
	// Verificar se jogador jù estù em alguma partida
	...
	// Criar estatùsticas do jogador
	auto stats = std::make_shared<s_arena7x7_player_stats>();

	// Validar se a alocaùùo foi bem sucedida
	if (!stats || !stats.get())
	{
	...
	// Garantir que o nome estù null-terminated antes de usar
	...
	// Capturar informaùùes da guild do jogador
	...
	if (sd->status.guild_id > 0)
	{
		...
		if (g && g->guild.name)
		{
			// Garantir que o nome da guild estù null-terminated
			...
			// Atualizar nome da guild do time se ainda nùo foi definido
	...
	// Incrementar contador de jogadores vivos para atualizaùùo do placar
 ...
 * A restauraùùo do tùmulo ù feita em pc_setpos quando o jogador muda de mapa
 */
bool arena7x7_remove_player(map_session_data *sd, bool deserter)
{
	...
	{
		return false; // Nùo estù em partida
	}
	...
	// Marcar como desertor se aplicùvel
...
// #endif
	...
	// Remover do mapa de jogadores (mas manter nas estatùsticas da partida para histùrico)
	arena7x7_player_match.erase(it);
...
bool arena7x7_finish_match(uint32 match_id, e_arena7x7_team winner_team, uint16 blue_score, uint16 red_score)
{
	...
	// Calcular duraùùo da partida em segundos
	...
	for (auto &kv : match->players)
	{
		...
		// Se time_alive ainda ù 0, significa que o jogador nùo morreu
		// Entùo o tempo vivo ù desde join_time atù agora (fim da partida)
...
// #endif
	...
	// NùO restaurar jogadores aqui - serùo restaurados em arena7x7_remove_player
	// quando o script teleportar cada jogador individualmente
	// Isso mantùm os jogadores como tùmulo atù o momento do teleporte
	...
	for (auto &kv : match->players)
	{
		...
		// Se ù desertor e jù foi penalizado (nùo estù mais no player_match), pular
		...
		{
// #if ARENA7X7_DEBUG
// 			ShowInfo("Arena7x7: Desertor %s jù foi penalizado, pulando atualizaùùo no finish\n",
// 					 stats->char_name.c_str());
// #endif
			continue; // Jù foi penalizado em arena7x7_remove_player
		}

		// Desertor ù sempre tratado como derrota, nùo importa o resultado do time
	...
	// NùO remover jogadores aqui - serùo removidos em pc_setpos quando teleportarem
	// Isso permite que a restauraùùo do tùmulo aconteùa corretamente
...
/**
 * Cancela uma partida
 * Se cancelada durante preparaùùo (antes de iniciar), remove completamente e libera o ID
 * NùO remove jogadores aqui - serùo removidos em pc_setpos quando teleportarem
 */
bool arena7x7_cancel_match(uint32 match_id)
{
	...
	// Se a partida ainda estava em preparaùùo (nunca iniciou), deletar completamente
	// para liberar o match_id e nùo "pular" IDs
	if (match->status == ARENA7X7_MATCH_WAITING)
	{
		...
		// Se for a ùltima partida criada, decrementar o contador para reutilizar o ID
...
// #endif
	...
	// Se a partida jù tinha iniciado, apenas marca como cancelada (mantùm no histùrico)
	match->status = ARENA7X7_MATCH_CANCELLED;
	match->end_time = gettick();

// #if ARENA7X7_DEBUG
// 	ShowInfo("Arena7x7: Partida %u cancelada apùs inùcio\n", match_id);
// #endif
	return true;
}
 ...
 * Pode ser chamado pelo NPC quando os times sùo formados
 */
...
/**
 * Verifica se um jogador estù em uma partida (ativa ou finalizada)
 * Inclui partidas finalizadas para permitir a restauraùùo do tùmulo no momento do warp
 */
bool arena7x7_is_in_match(uint32 char_id)
{
	...
	if (!match)
		return false;
	// Incluir FINISHED e CANCELLED para permitir restauraùùo do tùmulo no warp
 ...
 * Jogadores mortos/tumba nùo podem receber dispell, dano, etc.
 */
							...
							uint32 attacker_id, uint32 target_id, uint16 skill_id, uint32 damage, bool is_critical)
{
	...
	entry.is_kill = false; // Serù atualizado em record_kill se for o caso
	entry.timestamp = now;
	match->damage_log.push_back(entry);

	// Atualizar estatùsticas do atacante
	...
	// Atualizar estatùsticas do alvo
	...
	if (tit != match->players.end() && tit->second)
	{
		...
		// Registrar dano recente para assistùncias
		...
		// Limpar danos antigos (fora da janela de assistùncia)
	...
	if (skill_data.char_id == 0)
	{
		...
		// Nome da skill serù preenchido depois, se necessùrio
						  ...
						  uint32 killer_id, uint32 victim_id, uint16 skill_id, uint32 kill_damage)
{
	...
	// Buscar assistentes (quem causou dano recente na vùtima)
	...
	if (vit != match->players.end() && vit->second)
	{
		...
		// Coletar atacantes ùnicos (exceto o killer)
		...
		// Limpar danos recentes apùs morte
		vstats->recent_damage_taken.clear();

		// Atualizar estatùsticas da vùtima
		vstats->deaths++;
		vstats->current_streak = 0;
		vstats->last_death_time = now;

		// Calcular tempo vivo atù este momento (morte permanente - tumba)
	...
	// Atualizar estatùsticas do killer
							 ...
							 uint32 caster_id, uint32 target_id, uint16 skill_id, e_arena7x7_support_type type, uint32 value)
{
	...
	// Atualizar estatùsticas do caster
	...
	// Atualizar estatùsticas do alvo (se heal)
							  ...
							  e_arena7x7_item_type type, uint32 value)
{
	...
	// Atualizar estatùsticas do jogador
	...
	if (it != match->players.end() && it->second)
	{
		...
		// Verificar itens especùficos pelo ID
		switch (item_id)
		{
...
bool arena7x7_save_match(std::shared_ptr<s_arena7x7_match> match)
{
	...
	// Salvar log de dano detalhado (apenas os primeiros N para nùo sobrecarregar)
 ...
 * @param count_match - Se false, nùo incrementa contador de partidas (usado para desertores de partidas canceladas)
 */
bool arena7x7_update_ranking(uint32 char_id, s_arena7x7_player_stats *stats, bool won, bool lost, bool tie, bool count_match)
{
	...
	// Se o jogador ù desertor, aplica penalidade de -3 pontos e +1 death
...
// #endif
	...
	// Verificar se jogador jù existe no ranking
	if (SQL_ERROR == Sql_Query(mmysql_handle,
...
void arena7x7_update_score_display(std::shared_ptr<s_arena7x7_match> match)
{
	...
	// Enviar atualizaùùo para todos os jogadores no mapa via protocolo BG
...
// 	// OFICIAL: Player fica MORTO mas com disguise visÌvel
...
// 	// AGORA marcar como morto no sistema Arena7x7 (impede ressurreiÁ„o e dano futuros)
// 	// Fazer isso ANTES do disguise para que a proteùùo contra dano jù esteja ativa
// 	stats->is_dead = true;
// // #if ARENA7X7_DEBUG
...
// 	status_change_start(NULL, sd, SC_STOP, 10000, 0, 0, 0, 0, 1800000, SCSTART_NOAVOID | SCSTART_NOTICKDEF | SCSTART_NORATEDEF, 0);
	// Marcar como morto no sistema Arena7x7 (impede ressurreiÁ„o e dano futuros)
	stats->is_dead = true;
	...
	// ForÁar estado visual de morto (deitado)
	sd->state.dead_sit = 1;
	sd->vd.dead_sit = 1;

	// Parar qualquer movimento/aÁ„o
	unit_stop_walking(sd, USW_FIXPOS);
	unit_stop_attack(sd);
	...
	// Enviar packet para forÁar cliente mostrar jogador morto (deitado)
	...
	// O pc_dead() ù chamado ANTES de arena7x7_on_death em status_damage()
...
// #endif
	...
	if (stats)
	{
		stats->is_dead = false; // Permite aùùes novamente
	}

	// Remover status de imobilizaùùo
						...
						uint16 skill_id, int damage, bool is_critical)
{
	...
	// Verificar se ambos estùo na mesma partida
 ...
 * Registra a kill/morte nas estatùsticas e transforma o jogador em tumba.
 */
...
// #endif
	...
	// Verificar se o jogador jù estù morto (tumba) - evita processar duas vezes
...
// #endif
	...
	uint16 final_skill_id = skill_id; // skill_id=0 È v·lido (ataque normal)
				...
				if (DIFF_TICK(now, rd.timestamp) <= ARENA7X7_ASSIST_WINDOW)
				{
					...
					if (attacker_match && attacker_match->match_id == match->match_id)
					{
						if (rd.timestamp > most_recent)
						{
							...
							final_skill_id = rd.skill_id; // skill_id=0 È v·lido (ataque normal)
	...
	else
	{
		...
		if (vit != match->players.end() && vit->second && !vit->second->recent_damage_taken.empty())
		{
			// Buscar o ˙ltimo dano deste killer especÌfico
	...
	{
		// Apenas atualizar morte da vÌtima (morte por ambiente, queda, etc)
		auto vit = match->players.find(victim->status.char_id);
		if (vit != match->players.end() && vit->second)
		{
			vit->second->deaths++;
			...
			// Calcular tempo vivo atÈ este momento
			// Se j· tinha morrido antes, considera o tempo desde o join_time atÈ a ˙ltima morte
			// Como È morte permanente (tumba), sÛ morre uma vez
...
//custom moskaum
	// Se È um Shura com Cursed Circle ativo, remover de todos os alvos
...
//fim do custom
	...
	// Como retornamos 0 em pc_dead(), o cùdigo em status.cpp nùo chega a executar
	// o evento die_event, entùo precisamos disparù-lo aqui
	if (victim->bg_id)
	{
...
void arena7x7_on_heal(map_session_data *caster, map_session_data *target, uint16 skill_id, int heal_amount)
{
	...
	// Verificar se o alvo estù na mesma partida
	auto target_match = arena7x7_get_player_match(target->status.char_id);
	if (!target_match || target_match->match_id != match->match_id)
		return;

	// Verificar se sùo do mesmo time (heal em inimigo nùo conta como suporte)
...
void arena7x7_on_buff(map_session_data *caster, map_session_data *target, uint16 skill_id)
{
	...
	// Verificar se sùo do mesmo time
	if (!arena7x7_same_team(caster->status.char_id, target->status.char_id))
	{
		// ù debuff em inimigo
		arena7x7_record_support(match, caster->status.char_id, target->status.char_id,
...
void arena7x7_track_item_consume(map_session_data *sd, t_itemid item_id, const char *item_name, int amount)
{
	...
	// Lista de itens que sùo requisitos de skills (nùo contam para APM)
	// Esses itens sùo consumidos automaticamente ao usar skills
	...
	// Lista de flechas - sù contam para APM quando usadas em ataque bùsico (related_skill_id == 0)
	// Quando usadas por skills (Arrow Storm, etc), NùO contam para APM
	...
	// Lista de IDs de itens consumùveis a serem monitorados (para APM)
	static const std::unordered_set<t_itemid> tracked_items = {
		// Poùùes de HP bùsicas
		501,
		502,
		503,
		504,
	...
	// Verificar se ù um item que ù requisito de skill (log mas nùo conta APM)
	bool is_skill_requirement = (skill_requirement_items.find(item_id) != skill_requirement_items.end());

	// Verificar se ù uma flecha (tratamento especial)
	bool is_arrow = (arrow_items.find(item_id) != arrow_items.end());

	// Verificar se ù um item rastreùvel (conta para APM)
	bool is_tracked = (tracked_items.find(item_id) != tracked_items.end());

	// Se nùo ù nem requisito de skill, nem flecha, nem rastreùvel, ignorar
	...
	// Itens arremessùveis (Kunai, Shuriken, etc)
	...
	// Tambùm registrar no item_log antigo para estatùsticas
	for (int i = 0; i < amount; i++)
	{
		arena7x7_record_item_use(match, sd->status.char_id, 0, item_id, item_name, type, 1);
	}

	// Atualizar contador de itens usados (apenas para itens rastreados, nùo requisitos de skill)
	// Itens que sùo requisitos de skill (gemas, flechas, etc) nùo contam para APM
...
// ============================================================================
// Funùùes de Log Detalhado para o Site
// ============================================================================
							   ...
							   uint16 skill_id, uint16 skill_lv, int64 damage, int hit_count, bool is_critical)
{
	...
	if (!caster || !target || damage <= 0)
		return;
	if (target->type != BL_PC)
		return; // Sù logar dano em jogadores
	...
	// Verificar se sùo times diferentes (nùo logar dano em aliados)
 ...
 * Incrementa total_skills_used nas estatùsticas do jogador
										 ...
										 uint16 amount, uint16 skill_id, const char *item_type)
{
	...
	// Atualizar contador de itens usados nas estatùsticas
	auto stats = arena7x7_get_player_stats(sd->status.char_id);
	if (stats)
	{
 ...
 * @param skill_id - ID da skill que consumiu a flecha (0 = ataque bùsico)
 * Flechas sù contam para APM quando usadas em ataque bùsico (skill_id == 0)
 */
void arena7x7_log_arrow_consumption(map_session_data *sd, t_itemid arrow_id, uint16 amount, int32 skill_id)
{
	...
	// Atualizar contador de itens usados nas estatùsticas
	// APENAS se for ataque bùsico (skill_id == 0)
	// Flechas consumidas por skills (Arrow Storm, etc) NùO contam para APM
	if (skill_id == 0)
	{
		auto stats = arena7x7_get_player_stats(sd->status.char_id);
		if (stats)
	...
	// Criar entrada de log (sempre loga, independente de ser skill ou nùo)
	s_arena7x7_item_log log_entry;
	log_entry.char_id = sd->status.char_id;
	log_entry.char_name = sd->status.name;
...
{
	// Carregar prùximo match_id do banco
