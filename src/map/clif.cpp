...
static inline unsigned char clif_bl_type(struct block_list *bl, bool walking)
{
	...
	{
	case BL_PC:
		// Arena 7x7: Jogadores disfar�ados de t�mulo mantidos como mortos (dead_sit=2)
		if (((map_session_data *)bl)->disguise == MOBID_ARENA_TOMBSTONE)
		{
			return 0x0; // PC_TYPE (morto, n�o atac�vel)
		}
...
#endif
	case BL_PET:
		return pcdb_checkid(status_get_viewdata(bl)->look[LOOK_BASE]) ? 0x0 : 0x7; // NPC_PET_TYPE
	case BL_HOM:
...
/* Might be earlier, this is when the named item bug began */
	...
	{
#if PACKETVER >= 20091103
		// Arena 7x7: Jogadores disfar�ados de t�mulo (mortos, dead_sit=2)
		if (bl->type == BL_PC && ((map_session_data *)bl)->disguise == MOBID_ARENA_TOMBSTONE)
		{
			p.objecttype = 0x0; // PC_TYPE (morto, sem espada/bal�o)
		}
		else
		{
...
/// Special version of clif_skill_nodamage that bypasses clif_ally_only restrictions
/// Used for skills that need to show effects even when target is hidden (like strip skills)
bool clif_skill_nodamage_forced(block_list *src, block_list &dst, uint16 skill_id, int32 heal, bool success)
{
	PACKET_ZC_USE_SKILL p{};
	...
	// Send to all players in area manually, bypassing clif_ally_only restrictions
	map_foreachinarea([](struct block_list *bl, va_list ap) -> int
					  {
		if (bl->type != BL_PC) return 0;
		
		map_session_data *sd = (map_session_data*)bl;
		if (!sd || !session_isActive(sd->fd)) return 0;
		
		PACKET_ZC_USE_SKILL *p = va_arg(ap, PACKET_ZC_USE_SKILL*);
		
		WFIFOHEAD(sd->fd, sizeof(PACKET_ZC_USE_SKILL));
		memcpy(WFIFOP(sd->fd, 0), p, sizeof(PACKET_ZC_USE_SKILL));
		WFIFOSET(sd->fd, sizeof(PACKET_ZC_USE_SKILL));
		
		return 0; }, dst.m, dst.x - AREA_SIZE, dst.y - AREA_SIZE, dst.x + AREA_SIZE, dst.y + AREA_SIZE, BL_PC, &p);

	// Handle disguised targets
	if (disguised(&dst))
	{
		p.targetAID = disguised_bl_id(dst.id);
		clif_send(&p, sizeof(p), &dst, SELF);
...
void clif_sendegg(map_session_data *sd)
{
	...
	fd = sd->fd;
	if (battle_config.pet_no_gvg && map_flag_gvg2(sd->m))
...
void clif_viewequip_ack(map_session_data &sd, map_session_data &tsd)
{
	...
	for (int32 i = 0, equip = 0; i < EQI_MAX; i++)
	{
		...
		if (k >= 0)
		{
			if (tsd.inventory.u.items_inventory[k].nameid == 0 || tsd.inventory_data[k] == nullptr)
			{ // Item doesn't exist
				continue;
			}
...
void clif_parse_ActionRequest_sub(map_session_data &sd, uint8 action_type, int32 target_id, t_tick tick)
{
	...
	case DMG_REPEAT: // continuous attack
		...
		// Arena7x7: Verificar se o alvo � um jogador morto na arena
		{
			...
			if (target_bl && target_bl->type == BL_PC)
			{
				...
				{
					clif_displaymessage(sd.fd, "Este jogador est� eliminado e n�o pode ser atacado.");
					return;
				}
			}
		}
	...
	case DMG_SIT_DOWN: // sitdown
		...
		// custom moskaum pra poder sentar no exilio
		if (sd.ud.skilltimer != INVALID_TIMER || (sd.sc.opt1 && sd.sc.opt1 != OPT1_STONEWAIT && sd.sc.opt1 != OPT1_BURNING && sd.sc.opt1 != OPT1_IMPRISON))
			// fim do custom
			break;

		if (!sd.sc.empty() && (sd.sc.getSCE(SC_DANCING) ||
	...
	case DMG_STAND_UP: // standup
		...
		// if (sd.sc.opt1 && sd.sc.opt1 != OPT1_STONEWAIT && sd.sc.opt1 != OPT1_BURNING)
		// custom moskaum pra poder levantar no exilio
		if (sd.sc.opt1 && sd.sc.opt1 != OPT1_STONEWAIT && sd.sc.opt1 != OPT1_BURNING && sd.sc.opt1 != OPT1_IMPRISON)
			// fim do custom
			break;
...
void clif_parse_Restart(int32 fd, map_session_data *sd)
{
	...
	{
	case 0x00:
		// Arena7x7: Bloquear respawn de jogadores mortos na partida
		if (arena7x7_is_player_dead(sd->status.char_id))
		{
			clif_displaymessage(fd, "[Arena 7x7] Voce foi eliminado! Aguarde o fim da partida.");
			return; // Bloquear respawn
		}
		pc_respawn(sd, CLR_OUTSIGHT);
		break;
	case 0x01:
		// Arena7x7: Bloquear logout de jogadores na partida
		if (arena7x7_is_in_match(sd->status.char_id))
		{
			clif_displaymessage(fd, "[Arena 7x7] Voce nao pode sair durante a partida!");
			clif_disconnect_ack(sd, 1);
			return;
		}
		/*	Rovert's Prevent logout option - Fixed [Valaris]	*/
...
static void clif_parse_UseSkillToId_homun(struct homun_data *hd, map_session_data *sd, t_tick tick, uint16 skill_id, uint16 skill_lv, int32 target_id)
{
	...
	// Arena 7x7: Bloquear skills de homunculus em jogadores mortos
	if (hd->id != target_id)
	{
...
static void clif_parse_UseSkillToId_mercenary(s_mercenary_data *md, map_session_data *sd, t_tick tick, uint16 skill_id, uint16 skill_lv, int32 target_id)
{
	...
	// Arena 7x7: Bloquear skills de mercen�rio em jogadores mortos
	if (md->id != target_id)
	{
		map_session_data *tsd = map_id2sd(target_id);
		if (tsd && arena7x7_is_player_dead(tsd->status.char_id))
...
void clif_parse_skill_toid(map_session_data *sd, uint16 skill_id, uint16 skill_lv, int32 target_id)
{
	...
	// Arena 7x7: Bloquear uso de skills em jogadores mortos (exceto self skills)
	if (target_id != sd->id && !(inf & INF_SELF_SKILL))
	{
		map_session_data *tsd = map_id2sd(target_id);
		if (tsd && arena7x7_is_player_dead(tsd->status.char_id))
		{
...
///     3 = check password
/// NOTE: This packet is only available on certain non-kRO clients.
void clif_parse_StoragePassword(int32 fd, map_session_data *sd)
{ //@TODO
  // TODO: shuffle packet
  //	struct s_packet_db* info = &packet_db[RFIFOW(fd,0)];
...
//custom moskaum
/**
 * Send elemental HP bar to specific player
 * Similar to clif_monster_hp_bar but for elementals
 */
void clif_elemental_hp_bar(s_elemental_data *ed, int32 fd)
{
#if PACKETVER >= 20120404
	WFIFOHEAD(fd, packet_len(0x977));
	...
	WFIFOW(fd, 0) = 0x977;
	WFIFOL(fd, 2) = ed->id;
	WFIFOL(fd, 6) = ed->battle_status.hp;
	WFIFOL(fd, 10) = ed->battle_status.max_hp;
