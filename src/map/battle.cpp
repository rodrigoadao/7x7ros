...
int32 battle_damage(struct block_list *src, struct block_list *target, int64 damage, int16 div_, uint16 skill_lv, uint16 skill_id, enum damage_lv dmg_lv, uint16 attack_type, bool additional_effects, t_tick tick, bool isspdamage, bool is_norm_attacked)
{
	...
	// custom moskaum Similar system for elementals
	if (src != nullptr && target->type == BL_ELEM && dmg_lv > ATK_BLOCK)
	{
		s_elemental_data *ed = reinterpret_cast<s_elemental_data *>(target);
		if (ed && damage > 0)
		{
			elemental_damage(ed, src, damage);
		}
	}
	// Fim do custom
...
// 	}
//fim do original
//início do custom pra n ter bloqueio autoguard se redentado for do range do devotion
	if ((sce = sc->getSCE(SC_AUTOGUARD)) && flag & BF_WEAPON && rnd() % 100 < sce->val2 && !skill_get_inf2(skill_id, INF2_IGNOREAUTOGUARD))
	{
		status_change_entry *sce_d = sc->getSCE(SC_DEVOTION);
		block_list *d_bl;
		int32 delay;
		bool should_activate = true; // Flag to determine if autoguard should activate

		// Check if this is an inherited autoguard from devotion
		// If yes, check if devotion is in range before activating
		if (sce_d)
		{
			// Target is under devotion - verify range before allowing inherited autoguard to work
			d_bl = map_id2bl(sce_d->val1);
			
			if (d_bl && 
				((d_bl->type == BL_MER && ((TBL_MER *)d_bl)->master && ((TBL_MER *)d_bl)->master->id == target->id) ||
				(d_bl->type == BL_PC && ((TBL_PC *)d_bl)->devotion[sce_d->val2] == target->id)))
			{
				// Devotion relationship is valid - check distance
				if (!check_distance_bl(target, d_bl, sce_d->val3))
				{
					// Out of devotion range - inherited autoguard should NOT activate
		...
		// If should_activate is false, autoguard does not trigger - continue with normal damage calculation
	}
//fim do custom
	if (sc->getSCE(SC_NEUTRALBARRIER) && (((flag & (BF_LONG | BF_MAGIC)) == BF_LONG
										  ...
										  // Skills ranged do Mecânico também devem ser bloqueadas
										  skill_id == NC_VULCANARM ||
										  skill_id == NC_FLAMELAUNCHER ||
										  skill_id == NC_COLDSLOWER ||
										  skill_id == NC_ARMSCANNON))
	{
		d->dmg_lv = ATK_MISS;
		return false;
	}
	...
	if ((sce = sc->getSCE(SC_LIGHTNINGWALK)) && !(flag & BF_MAGIC) && flag & BF_LONG && rnd() % 100 < sce->val1)
	{
		...
		// custom moskaum pra n dar gapclose nunca
	...
	if ((sce = sc->getSCE(SC_KAUPE)) && (skill_id != NPC_EARTHQUAKE || (skill_id == NPC_EARTHQUAKE && flag & NPC_EARTHQUAKE_FLAG)) && rnd() % 100 < sce->val2)
...
int64 battle_calc_damage(struct block_list *src, struct block_list *bl, struct Damage *d, int64 damage, uint16 skill_id, uint16 skill_lv)
{
	...
	// Arena7x7: Jogadores "tumba" n�o podem receber dano
	if (bl->type == BL_PC) {
		map_session_data *target_sd = (map_session_data *)bl;
		if (arena7x7_is_player_dead(target_sd->status.char_id)) {
			return 0; // Bloquear todo dano
		}
	}
...
if (damage)
{
		if (tsc->getSCE(SC_DEEPSLEEP))
		{
			...
			// WM_METALICSOUND gets an additional 50% damage bonus against Deep Sleep targets
			...
			{
				struct block_list *d_bl = map_id2bl(tsc->getSCE(SC_DEVOTION)->val1);
				if (d_bl && check_distance_bl(bl, d_bl, tsc->getSCE(SC_DEVOTION)->val3))
...
// #endif
// 		{
// 			int32 reduction_percent = tsc->getSCE(SC_DEFENDER)->val2;
// 			int32 original_damage = damage;
// 			damage -= damage * reduction_percent / 100;
// 		}
//fim do original
//início do custom para não ter defender no redentado fora do range do devotion
if (tsc->getSCE(SC_DEFENDER) &&
	skill_id != NJ_ZENYNAGE && skill_id != KO_MUCHANAGE &&
#ifdef RENEWAL
	((flag & (BF_LONG | BF_WEAPON)) == (BF_LONG | BF_WEAPON) || skill_id == GN_FIRE_EXPANSION_ACID))
...
#endif
{
	status_change_entry *defender_sce = tsc->getSCE(SC_DEFENDER);
	bool should_reduce = true;
	
	// If target has devotion active, check range before applying any defender reduction
	// This applies to both inherited defender (val3=1) and the target's own defender (val3=0)
	status_change_entry *sce_d = tsc->getSCE(SC_DEVOTION);
	
	if (sce_d)
	{
		// Target has devotion - check devotion range before allowing defender
		...
		{
			// Devotion relationship is valid - check distance
			if (!check_distance_bl(bl, d_bl, sce_d->val3))
			{
				// Out of devotion range - defender should NOT reduce damage
				should_reduce = false;
			}
		}
		else
		{
			// Devotion caster not found or relationship invalid - defender should NOT work
			should_reduce = false;
		}
	}
	...
	if (should_reduce)
	{
		int32 reduction_percent = defender_sce->val2;
		int32 original_damage = damage;
		damage -= damage * reduction_percent / 100;
	}
}
//fim do custom
		...
		if (damage > 0 && (sce = tsc->getSCE(SC_STONEHARDSKIN)))
		{
			...
			// Quebra arma apenas com ataques básicos físicos de curta distância
			else if (skill_id == 0 && (flag & BF_WEAPON) && (flag & BF_SHORT))
...
static int64 battle_calc_base_damage(struct block_list *src, struct status_data *status, struct weapon_atk *wa, status_change *sc, uint16 t_size, int32 flag)
{
	...
	{ // PCs
		...
		if (!(flag & BDMG_CRIT) || (flag & BDMG_ARROW))
		{ // Normal attacks
			atkmin = status->dex;
...
void battle_consume_ammo(map_session_data *sd, int32 skill, int32 lv)
{
	...
	if (sd->equip_index[EQI_AMMO] >= 0) { // Qty check should have been done in skill_check_condition
		t_itemid arrow_id = sd->inventory.u.items_inventory[sd->equip_index[EQI_AMMO]].nameid;
		pc_delitem(sd, sd->equip_index[EQI_AMMO], qty, 0, 1, LOG_TYPE_CONSUME);
		
		// Arena7x7: Log de consumo de flechas/muni��o
		// Passa skill_id para diferenciar ataque b�sico (skill=0) de skills (skill>0)
		arena7x7_log_arrow_consumption(sd, arrow_id, (uint16)qty, skill);
	}
...
static int32 battle_range_type(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	case SS_SHIMIRU: // 11 cell cast range.
		return BF_SHORT;
	case CD_PETITIO:
	{ // Skill range is 2 but damage is melee with books and ranged with mace.
		map_session_data *sd = BL_CAST(BL_PC, src);
...
//fim original
//incío do custom moskaum pra fazer com que poema niff não vire espadinha
		if (su && su->group && su->group->skill_id == NPC_REVERBERATION)
...
static bool is_attack_hitting(struct Damage *wd, struct block_list *src, struct block_list *target, int32 skill_id, int32 skill_lv, bool first_call)
{
	...
	if (tsc && tsc->getSCE(SC_NEUTRALBARRIER) && ((wd->flag & (BF_LONG | BF_MAGIC)) == BF_LONG ||
												  // Skills ranged do Mecânico também devem ser bloqueadas
												  skill_id == NC_VULCANARM || skill_id == NC_FLAMELAUNCHER || skill_id == NC_COLDSLOWER || skill_id == NC_ARMSCANNON))
		return false;
	// Mecânico com Neutral Barrier ativo erra seus próprios ataques ranged
	// Exceção: se tiver o buff SC_STRIKING, a restrição é bypassada
	//if (sc && sc->getSCE(SC_NEUTRALBARRIER_MASTER) && !sc->getSCE(SC_STRIKING))
	// if (sc && sc->getSCE(SC_NEUTRALBARRIER) && !sc->getSCE(SC_STRIKING))
	// {
	// 	// Verificar se é um ataque ranged ou uma das skills específicas do Mecânico
	// 	if ((wd->flag & (BF_LONG | BF_MAGIC)) == BF_LONG ||
	// 		skill_id == NC_VULCANARM || skill_id == NC_FLAMELAUNCHER || 
	// 		skill_id == NC_COLDSLOWER || skill_id == NC_ARMSCANNON)
	// 	{
	// 		return false; // Ataque erra (MISS)
	// 	}
	// }
	if (sc && sc->getSCE(SC_NEUTRALBARRIER) && !sc->getSCE(SC_STRIKING) && skill_id == NC_VULCANARM)
    return false;
	flee = tstatus->flee;
#ifdef RENEWAL
...
static void battle_calc_damage_parts(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (sd && sd->sc.getSCE(SC_SEVENWIND))
...
#ifndef RENEWAL
			if (sd->bonus.crit_atk_rate && (bflag & BDMG_CRIT))
...
static int32 battle_calc_attack_skill_ratio(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	// fim do original
	// custom moskaum, com objetivo de fazer SD não ser buffado por força violenta e força violentíssima
	if (sc && skill_id != PA_SACRIFICE && skill_id != NC_SELFDESTRUCTION)
	/// fim do custom
	{
#ifdef RENEWAL
		...
		if (!skill_id || skill_id == KN_AUTOCOUNTER)
		{
			...
			{
				if (sc->getSCE(SC_GIANTGROWTH) && (sd->class_ & MAPID_THIRDMASK) == MAPID_RUNE_KNIGHT)
	...
	//  	break;
	//
	// custom com dano arrumado
	case RK_SONICWAVE:
		skillratio += -100 + 1050 + skill_lv;
		RE_LVL_DMOD(100);
		break;
	...
	//início do custom
		...
		if (sc && sc->getSCE(SC_INSPIRATION))
			...
			skillratio += -100 + 300 * skill_lv;
		skillratio += sstatus->vit / 6; // !TODO: What's the VIT bonus?
		skillratio /= 7; // Divide by hit count to get correct damage per hit
		RE_LVL_DMOD(100);
		break;
		//
	case SR_EARTHSHAKER:
	...
	// 	break;
	//fim original
	//início custom moskaum pra ajustar dano ruína
	case SR_FALLENEMPIRE:
	{
		// ATK [Base_Damage x (BaseLv + 100) + STR x 100]%
		const int base_damage[11] = {0, 400, 700, 1000, 1300, 1700, 2000, 2300, 2600, 2900, 3200};
		skillratio += -100 + base_damage[skill_lv] * (status_get_lv(src) + 100) / 100 + sstatus->str * 100;
	}
		break;
	//fim custom
	case SR_TIGERCANNON:
	...
	case SR_GATEOFHELL:
		if (sc && sc->getSCE(SC_COMBO) && sc->getSCE(SC_COMBO)->val1 == SR_FALLENEMPIRE)
			...
			skillratio += -100 + 500 * skill_lv;
		// edit moskaum, n tem pq ter mod de nv se todos serão nv170
		RE_LVL_DMOD(100);
		if (sc->getSCE(SC_GT_REVITALIZE))
			skillratio += skillratio * 30 / 100;
		break;
...
static void battle_calc_attack_left_right_hands(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (sd)
	{
		...
		else if (is_attack_left_handed(src, skill_id) && sd->status.weapon != W_KATAR)
		{ // Dual-wield
#else
		}
...
{
	if ((wd->damage + wd->damage2) && src != target && // Don't reflect your own damage (Grand Cross)
		(!skill_id || skill_id ||
		 (src->type == BL_SKILL && (skill_id == SG_SUN_WARM || skill_id == SG_MOON_WARM || skill_id == SG_STAR_WARM))))
	{
		...
		// When calculating reflect for GvG/BG reduced damage we must allow
		// status-based reflects (Reflect Shield). Pass 'true' so
		// battle_calc_return_damage considers SC_REFLECTSHIELD.
		rdamage = battle_calc_return_damage(target, src, &damage, wd->flag, skill_id, true);
		if (rdamage > 0)
		{ // Item reflect gets calculated after GVG/BG damage reduction is applied
			struct block_list *d_bl = battle_check_devotion(src);

			// Check for SC_VITALITYACTIVATION on the attacker (src)
			status_change *sc = status_get_sc(src);
			if (sc && sc->getSCE(SC_VITALITYACTIVATION))
			{
				rdamage /= 2;
...
static void battle_calc_weapon_final_atk_modifiers(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
		...
		rnd() % 100 < tsc->getSCE(SC_REJECTSWORD)->val2)
	{
		...
		//comentação custom by moskaum pra n ter animação do instinto de defesa, esperando feedback samu pra validar
	...
	// Check if target has DEVOTION active and within range before triggering CRESCENTELBOW
	bool devotion_active = false;
	if (tsc && tsc->getSCE(SC_DEVOTION) && tsc->getSCE(SC_DEVOTION)->val1)
	{
		block_list *d_bl = map_id2bl(tsc->getSCE(SC_DEVOTION)->val1);
		if (d_bl && check_distance_bl(target, d_bl, tsc->getSCE(SC_DEVOTION)->val3))
			devotion_active = true;
	}

	if (tsc && tsc->getSCE(SC_CRESCENTELBOW) && wd->flag & BF_SHORT && rnd() % 100 < tsc->getSCE(SC_CRESCENTELBOW)->val2 && !devotion_active)
	{
//custom moskaum pra fazer animação ao procar cotovelada
		// Show skill animation and name
		clif_skill_nodamage(target, *src, SR_CRESCENTELBOW_AUTOSPELL, tsc->getSCE(SC_CRESCENTELBOW)->val1, 1);
//fim do custom 
...
void battle_do_reflect(int32 attack_type, struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if ((wd->damage + wd->damage2) && src && target && src != target && (src->type != BL_SKILL || (src->type == BL_SKILL && (skill_id == SG_SUN_WARM || skill_id == SG_MOON_WARM || skill_id == SG_STAR_WARM))))
	{
		...
		// Skip reflection completely since battle_calc_attack_reflect_gvg_bg already handles ALL reflect cases
		// This prevents double reflection when attacking directly or through devotion
		bool should_reflect = false; // Disabled - all reflect handled by battle_calc_attack_reflect_gvg_bg

		if (should_reflect)
			rdamage = battle_calc_return_damage(target, src, &damage, wd->flag, skill_id, true);
		else
			rdamage = 0; // Ensure no reflection when disabled
		if (rdamage > 0)
		{
			struct block_list *d_bl = battle_check_devotion(src);
			status_change *sc = status_get_sc(src);
...
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int32 wflag)
{
	...
	{ // no need for math against plants
	  ...
	  // First call function with skill_id 0 to get base damage of a normal attack
		...
		if (tsd && skill_id != NJ_ISSEN && skill_id != GN_FIRE_EXPANSION_ACID)
		{ // These skills will do a card fix later
...
//fim do original
	...
	case SR_GATEOFHELL:
	{
		...
		// moskaum
		// double bonus = 1.0 + ((skill_lv * 2.0) / 10.0); // ou: 1.0 + skill_lv * 0.2
	...
	default:
		...
		// Note: Reflection moved after final modifiers to use correct damage
#ifdef RENEWAL
		break;
	}
#endif
		...
		default:
		{
			...
			case WM_METALICSOUND:
				...
				if (tsc && tsc->getSCE(SC_DEEPSLEEP))
					skillratio += 50; // !TODO: Confirm target sleeping bonus
				RE_LVL_DMOD(100);
				if (tsc && tsc->getSCE(SC_SOUNDBLEND))
					skillratio += skillratio * 50 / 100;
				break;
...
struct Damage battle_calc_misc_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int32 mflag)
{
	...
	case NJ_ISSEN:
		...
		// modified def formula
		{
			...
			if (sc && sc->getSCE(SC_BUNSINJYUTSU) && (i = sc->getSCE(SC_BUNSINJYUTSU)->val2) > 0)
...
int64 battle_calc_return_damage(struct block_list *tbl, struct block_list *src, int64 *dmg, int32 flag, uint16 skill_id, bool status_reflect, bool skip_mapflag_reduction)
{
	...
	{ // Bounces back part of the damage.
		...
		if (status_reflect && tsc != nullptr && !tsc->empty() && tsc->getSCE(SC_REFLECTSHIELD) && skill_id != WS_CARTTERMINATION && skill_id != NPC_MAXPAIN_ATK)
		{
			...
			// Por padrão, usa o status do próprio alvo (Professor)
			...
			if (reflect_sce)
			{
				...
				{
					// Calcula o dano de reflexão usando o val2 (porcentagem) do refletor correto
					// reflect_sce->val2 aqui será os 40% do Paladino, se ele for a fonte.
					rdamage += damage * reflect_sce->val2 / 100;
				}
			}
		}
		if (tsc && tsc->getSCE(SC_DEATHBOUND) && skill_id != WS_CARTTERMINATION && skill_id != GN_HELLS_PLANT_ATK && !status_bl_has_mode(src, MD_STATUSIMMUNE))
		{
	...
	{
		// Reflect damage should not be capped by target's HP since target is not receiving the reflect damage
		// The reflect damage hits the attacker (src), not the original target (tbl)
		return rdamage;
	}
}
...
enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, t_tick tick, int32 flag)
{
	...
	{
		if (sc && sc->getSCE(SC_DUPLELIGHT) && (wd.flag & BF_SHORT))
	...
	// Arena7x7: Log de ataque normal
	if (sd && target->type == BL_PC) {
		bool is_miss = (wd.dmg_lv == ATK_FLEE || damage <= 0);
		bool is_crit = (wd.type == DMG_CRITICAL);
		arena7x7_log_normal_attack(sd, target, damage, is_crit, is_miss);
	}
...
//a
	...
	// 	else
	// 	{
	// 	// Check Devotion BEFORE calling battle_delay_damage
	// 	if (tsc && tsc->getSCE(SC_DEVOTION))
	// 	{
	// 		struct status_change_entry *sce = tsc->getSCE(SC_DEVOTION);
	// 		struct block_list *d_bl = map_id2bl(sce->val1);
			
	// 		// Verify if devotion caster exists and is in range
	// 		if (d_bl && 
	// 			((d_bl->type == BL_MER && ((TBL_MER *)d_bl)->master && ((TBL_MER *)d_bl)->master->id == target->id) || 
	// 			 (d_bl->type == BL_PC && ((TBL_PC *)d_bl)->devotion[sce->val2] == target->id)) && 
	// 			check_distance_bl(target, d_bl, sce->val3))
	// 		{
	// 			// Devotion is IN RANGE - redirect damage to Paladin
	// 			if (damage > 0)
	// 			{
	...
	// 			}
	// 			// Damage was redirected to Paladin - do NOT call battle_delay_damage for target
	// 		}
	// 		else
	// 		{
	// 			// Devotion is OUT OF RANGE or caster is invalid - remove devotion and apply damage to target
	...
	// 	else
	// 	{
	// 		// No Devotion - apply damage normally via battle_delay_damage
	// 		battle_delay_damage(tick, wd.amotion, src, target, wd.flag, 0, 0, damage, wd.dmg_lv, wd.div_, true, wd.isspdamage, is_norm_attacked);
	// 	}
	// }
	// map_freeblock_lock();
	...
	// // 		// else
	// // 		// {
	// // 		// 	// Devotion is out of range - remove it and apply damage to the original target
	// // 		// 	status_change_end(target, SC_DEVOTION);
				
	// // 		// 	// Apply the damage to the target since devotion is not active
	// // 		// 	if (damage > 0)
	// // 		// 		battle_delay_damage(tick, wd.amotion, src, target, wd.flag, 0, 0, damage, wd.dmg_lv, wd.div_, true, wd.isspdamage, is_norm_attacked);
	// // 		// }
	// // 		else
	// // 		{
	// // 			// Devotion is out of range - remove it and ensure damage is applied
	// // 			status_change_end(target, SC_DEVOTION);
				
	// // 			// Force immediate damage application since devotion is invalid
	// // 			if (damage > 0)
	// // 			{
	// // 				// The battle_delay_damage at line 11978 might not work correctly after devotion removal
	// // 				// So we force immediate damage here
	// // 				map_freeblock_unlock();  // Unlock before applying damage
	// // 				battle_fix_damage(src, target, damage, wd.div_, 0);
	// // 				map_freeblock_lock();  // Re-lock
	// // 			}
	// // 		}
	// // 	}
	// 	//
	// else
	// {
	// 	// Verificar Devotion ANTES de chamar battle_delay_damage
	// 	if (tsc && tsc->getSCE(SC_DEVOTION))
	// 	{
	// 		struct status_change_entry *sce = tsc->getSCE(SC_DEVOTION);
	// 		struct block_list *d_bl = map_id2bl(sce->val1);
	...
	// 		else
	// 		{
	// 			// Devotion is OUT OF RANGE - remove it and apply damage to target
	// 			status_change_end(target, SC_DEVOTION);
	// 			battle_delay_damage(tick, wd.amotion, src, target, wd.flag, 0, 0, damage, wd.dmg_lv, wd.div_, true, wd.isspdamage, is_norm_attacked);
	// 		}
	// 	}
	// 	else
	// 	{
	// 		// No Devotion - apply damage normally
	// 		battle_delay_damage(tick, wd.amotion, src, target, wd.flag, 0, 0, damage, wd.dmg_lv, wd.div_, true, wd.isspdamage, is_norm_attacked);
	// 	}
	// 	}
	...
	else
	{
		...
		if (tsc && tsc->getSCE(SC_DEVOTION))
		{
			...
			{
				// Devotion is OUT OF RANGE or caster is invalid - remove devotion and apply damage to target IMMEDIATELY
				status_change_end(target, SC_DEVOTION);
				
				// Apply damage immediately instead of using battle_delay_damage
				// because battle_delay_damage might have issues after removing devotion
				if (damage > 0)
				{
					battle_damage(src, target, damage, wd.div_, 0, 0, wd.dmg_lv, wd.flag, true, tick, wd.isspdamage, is_norm_attacked);
				}
			}
		}
		else
...
//custom moskaum pra poema de nif n ser targetável
	// WM_POEMOFNETHERWORLD cannot be targeted/attacked
	if (target->type == BL_SKILL) {
		TBL_SKILL *su = (TBL_SKILL *)target;
		if (su && su->group && su->group->skill_id == WM_POEMOFNETHERWORLD)
			return -1; // Cannot be targeted
	}
//
	// Arena7x7: Jogadores "tumba" n�o podem ser alvo de ataques
	if (target->type == BL_PC) {
		map_session_data *target_sd = (map_session_data *)target;
		if (arena7x7_is_player_dead(target_sd->status.char_id)) {
			return -1; // N�o pode ser alvo
		}
	}
	...
	case BL_SKILL:
	{
		...
		if (skill_get_inf2(su->group->skill_id, INF2_ISTRAP) && su->group->unit_id != UNT_USED_TRAPS)
		{
			...
			else if (skill_get_inf2(skill_id, INF2_TARGETTRAP) || skill_id == HT_REMOVETRAP || skill_id == MA_REMOVETRAP)
			{ // Only a few skills can target traps
				// HT_REMOVETRAP and MA_REMOVETRAP can target any trap including enemy traps
				if (skill_id == HT_REMOVETRAP || skill_id == MA_REMOVETRAP)
				{
					// Allow targeting any trap
				}
				else
					switch (skill_id)
					{
		...
		else if (su->group->skill_id == WZ_ICEWALL || (su->group->skill_id == GN_WALLOFTHORN && skill_id != GN_CARTCANNON))
		{
			...
			default:
				...
				if ((flag & BCT_ALL) == BCT_ALL && !skill_get_inf2(skill_id, INF2_TARGETTRAP) && skill_id != HT_REMOVETRAP && skill_id != MA_REMOVETRAP)
					return -1;
			}
			state |= BCT_ENEMY;
			strip_enemy = 0;
