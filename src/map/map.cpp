...
int32 map_moveblock(struct block_list *bl, int32 x1, int32 y1, t_tick tick)
{
	...
	if (bl->type & BL_CHAR)
	{
		...
		if (bl->type == BL_PC && ((TBL_PC *)bl)->shadowform_id)
		...
		if (sc != nullptr && !sc->empty())
		{
			...
			else
			{
				...
				if (sc->getSCE(SC_NEUTRALBARRIER_MASTER) && !sideslide_movement_active)
				{
					if (neutralbarrier_needs_reposition)
					{
						// Reposition barrier to player center after sideslide
						std::shared_ptr<s_skill_unit_group> sg = skill_id2group(sc->getSCE(SC_NEUTRALBARRIER_MASTER)->val2);
						if (sg && sg->unit_count > 0)
						{
							// Calculate center position of the skill unit group
							int16 center_x = 0, center_y = 0;
							int16 active_units = 0;

							for (int i = 0; i < sg->unit_count; i++)
							{
								if (sg->unit[i].alive)
								{
									center_x += sg->unit[i].x;
									center_y += sg->unit[i].y;
									active_units++;
								}
							}
							...
							if (active_units > 0)
							{
								center_x /= active_units;
								center_y /= active_units;

								// Calculate difference between current group center and player position
								int16 dx = bl->x - center_x;
								int16 dy = bl->y - center_y;
								skill_unit_move_unit_group(sg, bl->m, dx, dy);
							}
							neutralbarrier_needs_reposition = false;
						}
					}
					else
					{
				...
				else if (sc->getSCE(SC_STEALTHFIELD_MASTER) && !sideslide_movement_active)
				{
					if (neutralbarrier_needs_reposition)
					{
						// Reposition stealth field to player center after sideslide
						std::shared_ptr<s_skill_unit_group> sg = skill_id2group(sc->getSCE(SC_STEALTHFIELD_MASTER)->val2);
						if (sg && sg->unit_count > 0)
						{
							// Calculate center position of the skill unit group
							int16 center_x = 0, center_y = 0;
...
int32 map_count_oncell(int16 m, int16 x, int16 y, int32 type, int32 flag)
{
	...
	if (type & ~BL_MOB)
		for (bl = mapdata->block[bx + by * mapdata->bxs]; bl != nullptr; bl = bl->next)
			...
			{
				if (bl->type == BL_NPC)
				{ // Don't count hidden or invisible npc. Cloaked npc are counted
...
const char *map_mapid2mapname(int32 m)
{
	...
	{ // Instance map check
		...
		else
		{
			...
			{ // Loop to find the src map we want
				if (it.m == m)
...
void map_setcell(int16 m, int16 x, int16 y, cell_t cell, bool flag)
{
	...
	{
	case CELL_WALKABLE:
		mapdata->cell[j].walkable = flag;
		break;
	case CELL_SHOOTABLE:
		mapdata->cell[j].shootable = flag;
		break;
	case CELL_WATER:
		mapdata->cell[j].water = flag;
		break;

	case CELL_NPC:
		mapdata->cell[j].npc = flag;
		break;
	case CELL_BASILICA:
		mapdata->cell[j].basilica = flag;
		break;
	case CELL_LANDPROTECTOR:
		mapdata->cell[j].landprotector = flag;
		break;
	case CELL_NOVENDING:
		mapdata->cell[j].novending = flag;
		break;
	case CELL_NOCHAT:
		mapdata->cell[j].nochat = flag;
		break;
	case CELL_MAELSTROM:
		mapdata->cell[j].maelstrom = flag;
		break;
	case CELL_ICEWALL:
		mapdata->cell[j].icewall = flag;
		break;
	case CELL_NOBUYINGSTORE:
		mapdata->cell[j].nobuyingstore = flag;
		break;
	default:
...
bool map_setmapflag_sub(int16 m, enum e_mapflag mapflag, bool status, union u_mapflag_args *args)
{
	...
	case MF_SKILL_DAMAGE:
		...
		else
		{
			...
			if (!args->flag_val)
			{ // Signifies if it's a single skill or global damage adjustment
...
void map_do_init_msg(void)
{
	...
	while (test != -1 && size > i)
	{ // for all enable lang +(English default)
...
int32 map_msg_config_read(const char *cfgName, int32 lang)
{
	...
	if (_msg_config_read(cfgName, MAP_MAX_MSG, mdb->msg) != 0)
	{								  // an error occur
