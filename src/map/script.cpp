...
const char *parse_variable(const char *p)
{
	...
	if (type == C_ADD_POST || type == C_SUB_POST)
	{ // post ++ / --
		add_scripti(1);
	...
	else if (type == C_ADD_PRE || type == C_SUB_PRE)
	{ // pre ++ / --
		add_scripti(1);
...
const char *parse_syntax(const char *p)
{
	...
	case 'I':
		if (p2 - p == 2 && !strncasecmp(p, "if", 2))
		{
			...
			p = skip_space(p2);
			if (*p != '(')
			{ // Prevent if this {} non-c syntax. from Rayce (jA)
				disp_error_message("need '('", p);
			}
...
struct script_data *get_val_(struct script_state *st, struct script_data *data, map_session_data *sd)
{
	...
	{
		if (sd == nullptr && !script_rid2sd(sd))
	...
	{ // string variable
		...
		if (data->u.str == nullptr || data->u.str[0] == '\0')
		{ // empty string
			data->type = C_CONSTSTR;
			data->u.str = const_cast<char *>("");
		}
...
void op_2num(struct script_state *st, int32 op, int64 i1, int64 i2)
{
	...
	default:
		...
		{
		case C_ADD:
			overflow = util::safe_addition(i1, i2, ret);
			break;
		case C_SUB:
			overflow = util::safe_substraction(i1, i2, ret);
			break;
		case C_MUL:
			overflow = util::safe_multiplication(i1, i2, ret);
			break;
		default:
		...
		case WARPPARTY_RANDOMALLAREA:
			if (!mapdata->getMapFlag(MF_NORETURN) && !mapdata->getMapFlag(MF_NOWARP) && pc_job_can_entermap((enum e_job)pl_sd->status.class_, m, pc_get_group_level(pl_sd)))
			{
				...
				if (rx || ry)
				{
					...
					if (attempts == 0)
					{ // Keep the original coordinates if fails to find a valid cell within the range
						nx = x;
						ny = y;
					}
				}
...
BUILDIN_FUNC(deletearray)
{
	...
	if (!(sa = static_cast<script_array *>(idb_get(src->arrays, id))))
...
BUILDIN_FUNC(getpartyleader)
{
	...
	case 3:
		...
		break;
	case 4:
		script_pushstrcopy(st, p->party.member[i].map);
		break;
	case 5:
...
BUILDIN_FUNC(bonus)
{
	...
	case SP_SUB_SKILL:
		...
		else
		{
			...
			if (strcmpi(script_getfuncname(st), "bonus") && !skill_get_index(val1))
			{ // Only check skill ID for bonus2, bonus3, bonus4, or bonus5
...
// in�cio do custom
//  ???????????????????????????????????????????????????????????????????????????
//  C�DIGO PARA ADICIONAR NO script.cpp
//  Localiza��o: Adicionar AP�S a fun��o plagiarizeskillreset (linha ~10352)
//  ???????????????????????????????????????????????????????????????????????????

/// Copia uma skill para o slot de Plagiarism (RG_PLAGIARISM)
/// Similar ao sistema de c�pia em combate, mas pode ser usado por NPCs
///
/// copyskill_plagiarism <skill id>,<level>
/// copyskill_plagiarism "<skill name>",<level>
///
/// Retorna 1 em sucesso, 0 em falha
BUILDIN_FUNC(copyskill_plagiarism)
{
	TBL_PC *sd;
	...
	// Verifica se o jogador tem RG_PLAGIARISM
	if (pc_checkskill(sd, RG_PLAGIARISM) == 0)
	{
		ShowWarning("copyskill_plagiarism: Player %s (AID: %d) doesn't have RG_PLAGIARISM skill.\n", sd->status.name, sd->status.account_id);
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	// Obt�m skill ID e level
	if (script_isstring(st, 2))
	...
	if (!skill_get_index(skill_id))
	{
		ShowWarning("copyskill_plagiarism: Invalid skill id %d\n", skill_id);
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	// Limita o n�vel ao n�vel de RG_PLAGIARISM do jogador
	skill_lv = min(skill_lv, pc_checkskill(sd, RG_PLAGIARISM));

	// Remove skill reproduzida se for a mesma
	if (sd->reproduceskill_idx &&
		sd->status.skill[sd->reproduceskill_idx].flag == SKILL_FLAG_PLAGIARIZED &&
		sd->status.skill[sd->reproduceskill_idx].id == skill_id)
	{
		// Remove Reproduce
		if (sd->reproduceskill_idx)
		{
...
///
/// Retorna 1 em sucesso, 0 em falha
BUILDIN_FUNC(copyskill_reproduce)
{
	TBL_PC *sd;
	uint16 skill_id, skill_lv;
	int idx;
	...
	// Verifica se o jogador tem SC_REPRODUCE
	if (pc_checkskill(sd, SC_REPRODUCE) == 0)
	{
		ShowWarning("copyskill_reproduce: Player %s (AID: %d) doesn't have SC_REPRODUCE skill.\n", sd->status.name, sd->status.account_id);
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
	...
	if (!skill_get_index(skill_id))
	{
		ShowWarning("copyskill_reproduce: Invalid skill id %d\n", skill_id);
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	// Limita o n�vel ao m�ximo da skill
	...
	// Remove skill reproduzida antiga
	if (sd->reproduceskill_idx)
	{
		sd->status.skill[sd->reproduceskill_idx].id = 0;
		sd->status.skill[sd->reproduceskill_idx].lv = 0;
...
/// Remove a skill copiada do slot de Plagiarism
///
/// removeskill_plagiarism
///
/// Retorna 1 em sucesso, 0 se n�o havia skill para remover
...
/// Remove a skill copiada do slot de Reproduce
///
...
BUILDIN_FUNC(removeskill_reproduce)
{
	...
	// Verifica se h� skill reproduzida
...
BUILDIN_FUNC(strmobinfo)
{
	...
	case 1:
		...
		break;
	case 2:
		script_pushstrcopy(st, mob->jname.c_str());
		break;
	case 3:
	...
	case 4:
		...
		break;
	case 5:
		script_pushint(st, mob->status.max_sp);
		break;
	case 6:
...
BUILDIN_FUNC(getiteminfo)
{
	...
	{
	case ITEMINFO_BUY:
		script_pushint(st, i_data->value_buy);
		break;
	case ITEMINFO_SELL:
		script_pushint(st, i_data->value_sell);
		break;
	case ITEMINFO_TYPE:
		script_pushint(st, i_data->type);
		break;
	case ITEMINFO_MAXCHANCE:
		script_pushint(st, i_data->maxchance);
		break;
	case ITEMINFO_GENDER:
		script_pushint(st, i_data->sex);
		break;
	case ITEMINFO_LOCATIONS:
		script_pushint(st, i_data->equip);
		break;
	case ITEMINFO_WEIGHT:
		script_pushint(st, i_data->weight);
		break;
	case ITEMINFO_ATTACK:
		script_pushint(st, i_data->atk);
		break;
	case ITEMINFO_DEFENSE:
		script_pushint(st, i_data->def);
		break;
	case ITEMINFO_RANGE:
		script_pushint(st, i_data->range);
		break;
	case ITEMINFO_SLOT:
		script_pushint(st, i_data->slots);
		break;
	case ITEMINFO_VIEW:
		if (i_data->type == IT_WEAPON || i_data->type == IT_AMMO)
		...
		break;
	case ITEMINFO_EQUIPLEVELMIN:
		script_pushint(st, i_data->elv);
		break;
	...
	case ITEMINFO_ARMORLEVEL:
		...
		break;
	case ITEMINFO_ALIASNAME:
		script_pushint(st, i_data->view_id);
		break;
	case ITEMINFO_EQUIPLEVELMAX:
		script_pushint(st, i_data->elvmax);
		break;
	...
	case ITEMINFO_ID:
		script_pushint(st, i_data->nameid);
		break;
	case ITEMINFO_AEGISNAME:
		script_pushstrcopy(st, i_data->name.c_str());
		break;
...
BUILDIN_FUNC(setiteminfo)
{
	...
	{
	case ITEMINFO_BUY:
		i_data->value_buy = static_cast<uint32>(value);
		break;
	case ITEMINFO_SELL:
		i_data->value_sell = static_cast<uint32>(value);
		break;
	case ITEMINFO_TYPE:
		i_data->type = static_cast<item_types>(value);
		break;
	case ITEMINFO_MAXCHANCE:
		i_data->maxchance = static_cast<int32>(value);
		break;
	case ITEMINFO_GENDER:
		i_data->sex = static_cast<uint8>(value);
		break;
	case ITEMINFO_LOCATIONS:
		i_data->equip = static_cast<uint32>(value);
		break;
	case ITEMINFO_WEIGHT:
		i_data->weight = static_cast<uint32>(value);
		break;
	case ITEMINFO_ATTACK:
		i_data->atk = static_cast<uint32>(value);
		break;
	case ITEMINFO_DEFENSE:
		i_data->def = static_cast<uint32>(value);
		break;
	case ITEMINFO_RANGE:
		i_data->range = static_cast<uint16>(value);
		break;
	case ITEMINFO_SLOT:
		i_data->slots = static_cast<uint16>(value);
		break;
	case ITEMINFO_VIEW:
		if (i_data->type == IT_WEAPON || i_data->type == IT_AMMO)
		{ // keep old compatibility
		...
		break;
	case ITEMINFO_EQUIPLEVELMIN:
		i_data->elv = static_cast<uint16>(value);
		break;
	case ITEMINFO_WEAPONLEVEL:
		if (i_data->type == IT_WEAPON)
		{
	...
	case ITEMINFO_ALIASNAME:
		i_data->view_id = static_cast<t_itemid>(value);
		break;
	case ITEMINFO_EQUIPLEVELMAX:
		i_data->elvmax = static_cast<uint16>(value);
		break;
	case ITEMINFO_MAGICATTACK:
	{
#ifdef RENEWAL
...
BUILDIN_FUNC(recovery)
{
	...
	case 1:
	{
		...
		int32 p_id = 0, i;
		if (script_hasdata(st, 5))
		{ // Bad maps shouldn't cause issues
...
BUILDIN_FUNC(getpetinfo)
{
	...
	case PETINFO_RENAMED:
		...
		break;
	case PETINFO_LEVEL:
		script_pushint(st, (int32)pd->pet.level);
		break;
	case PETINFO_BLOCKID:
		script_pushint(st, pd->id);
		break;
	case PETINFO_EGGID:
		script_pushint(st, pd->pet.egg_id);
		break;
	case PETINFO_FOODID:
		script_pushint(st, pd->get_pet_db()->FoodID);
		break;
	default:
		script_pushint(st, 0);
		break;
...
BUILDIN_FUNC(getmercinfo)
{
	...
	case 1:
		...
		break;
	case 2:
		script_pushstrcopy(st, md->db->name.c_str());
		break;
	case 3:
		script_pushint(st, mercenary_get_faith(md));
		break;
	case 4:
		script_pushint(st, mercenary_get_calls(md));
		break;
	case 5:
		script_pushint(st, md->mercenary.kill_count);
		break;
	case 6:
		script_pushint(st, mercenary_get_lifetime(md));
		break;
	case 7:
...
BUILDIN_FUNC(getsavepoint)
{
	...
	{
	case 0:
		script_pushstrcopy(st, sd->status.save_point.map);
		break;
	case 1:
...
BUILDIN_FUNC(isequipped)
{
	...
	if (!script_rid2sd(sd))
	{ // If the player is not attached it is a script error anyway... but better prevent the map server from crashing...
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(setbattleflag)
{
	...
	else
	{
		...
		if (script_hasdata(st, 4) && script_getnum(st, 4))
		{ // Only attempt to reload monster data if told to
...
BUILDIN_FUNC(explode)
{
	...
	while (str[i] != '\0')
	{
		if (str[i] == delimiter && start < SCRIPT_MAX_ARRAYSIZE - 1)
...
BUILDIN_FUNC(callshop)
{
	...
	if (nd->subtype == NPCTYPE_SHOP)
	{
		...
		{
		case 1:
			npc_buysellsel(sd, nd->id, 0);
			break; // Buy window
		case 2:
			npc_buysellsel(sd, nd->id, 1);
			break; // Sell window
		default:
...
BUILDIN_FUNC(delmonsterdrop)
{
	...
	if (mob)
	{ // We got a valid monster, check for item drop on monster
		bool found = false;
...
BUILDIN_FUNC(getmonsterinfo)
{
	...
	case MOB_MDEF:
		...
		break;
	case MOB_RES:
		script_pushint(st, mob->status.res);
		break;
	case MOB_MRES:
		script_pushint(st, mob->status.mres);
		break;
	case MOB_STR:
	...
	case MOB_LUK:
		...
		break;
	case MOB_SPEED:
		script_pushint(st, mob->status.speed);
		break;
	case MOB_ATKRANGE:
	...
	case MOB_ELEMENT:
		...
		break;
	case MOB_ELEMENTLV:
		script_pushint(st, mob->status.ele_lv);
		break;
	case MOB_MODE:
...
BUILDIN_FUNC(rid2name)
{
	...
	if ((bl = map_id2bl(rid)))
	{
		...
		case BL_HOM:
			...
			break;
		case BL_MER:
			script_pushstrcopy(st, ((TBL_MER *)bl)->db->name.c_str());
			break;
		default:
...
BUILDIN_FUNC(setunitdata)
{
	...
	case BL_MOB:
	{
		...
		{
		case UMOB_SIZE:
			md->status.size = md->base_status->size = (unsigned char)value;
			break;
		case UMOB_LEVEL:
		...
		case UMOB_MODE:
			...
			break;
		case UMOB_AI:
			md->special_state.ai = (enum mob_ai)value;
			break;
		case UMOB_SCOPTION:
			md->sc.option = (uint16)value;
			break;
		case UMOB_SEX:
		...
		case UMOB_CLASS:
			...
			break;
		case UMOB_HAIRSTYLE:
			clif_changelook(bl, LOOK_HAIR, (uint16)value);
			break;
		case UMOB_HAIRCOLOR:
			clif_changelook(bl, LOOK_HAIR_COLOR, (uint16)value);
			break;
		case UMOB_HEADBOTTOM:
			clif_changelook(bl, LOOK_HEAD_BOTTOM, (uint16)value);
			break;
		case UMOB_HEADMIDDLE:
			clif_changelook(bl, LOOK_HEAD_MID, (uint16)value);
			break;
		case UMOB_HEADTOP:
			clif_changelook(bl, LOOK_HEAD_TOP, (uint16)value);
			break;
		case UMOB_CLOTHCOLOR:
			clif_changelook(bl, LOOK_CLOTHES_COLOR, (uint16)value);
			break;
		case UMOB_SHIELD:
			clif_changelook(bl, LOOK_SHIELD, (uint16)value);
			break;
		case UMOB_WEAPON:
			clif_changelook(bl, LOOK_WEAPON, (uint16)value);
			break;
		case UMOB_LOOKDIR:
			unit_setdir(bl, (uint8)value);
			break;
		case UMOB_CANMOVETICK:
			md->ud.canmove_tick = value > 0 ? (uint32)value : 0;
			break;
		case UMOB_STR:
		...
		case UMOB_CRIT:
			...
			break;
		case UMOB_RACE:
			md->status.race = md->base_status->race = (unsigned char)value;
			break;
		case UMOB_ELETYPE:
		...
		case UMOB_ROBE:
			clif_changelook(bl, LOOK_ROBE, (uint16)value);
			break;
		case UMOB_BODY2:
			clif_changelook(bl, LOOK_BODY2, (uint16)value);
			break;
		case UMOB_GROUP_ID:
			md->ud.group_id = value;
			unit_refresh(bl);
			break;
		case UMOB_IGNORE_CELL_STACK_LIMIT:
			md->ud.state.ignore_cell_stack_limit = value > 0;
			break;
		case UMOB_RES:
		...
		case UMOB_MRES:
			...
			calc_status = true;
			break;
		case UMOB_DAMAGETAKEN:
			md->damagetaken = (uint16)value;
			break;
		default:
	...
	case BL_HOM:
	{
		...
		{
		case UHOM_SIZE:
			hd->battle_status.size = hd->base_status.size = (unsigned char)value;
			break;
		case UHOM_LEVEL:
			hd->homunculus.level = (uint16)value;
			break;
		case UHOM_HP:
		...
		case UHOM_MAXSP:
			...
			break;
		case UHOM_MASTERCID:
			hd->homunculus.char_id = (uint32)value;
			break;
		case UHOM_MAPID:
			if (mapname)
				value = map_mapname2mapid(mapname);
		...
		case UHOM_LOOKDIR:
			unit_setdir(bl, (uint8)value);
			break;
		case UHOM_CANMOVETICK:
			hd->ud.canmove_tick = value > 0 ? (uint32)value : 0;
			break;
		case UHOM_STR:
		...
		case UHOM_CRIT:
			...
			break;
		case UHOM_RACE:
			hd->battle_status.race = hd->base_status.race = (unsigned char)value;
			break;
		case UHOM_ELETYPE:
	...
	case BL_PET:
	{
		...
		{
		case UPET_SIZE:
			pd->status.size = (unsigned char)value;
			break;
		case UPET_LEVEL:
			pd->pet.level = (uint16)value;
			break;
		case UPET_HP:
		...
		case UPET_MAXHP:
			...
			break;
		case UPET_MASTERAID:
			pd->pet.account_id = (uint32)value;
			break;
		case UPET_MAPID:
			if (mapname)
				value = map_mapname2mapid(mapname);
		...
		case UPET_LOOKDIR:
			unit_setdir(bl, (uint8)value);
			break;
		case UPET_CANMOVETICK:
			pd->ud.canmove_tick = value > 0 ? (uint32)value : 0;
			break;
		case UPET_STR:
		...
		case UPET_LUK:
			...
			break;
		case UPET_DMGIMMUNE:
			pd->ud.immune_attack = value > 0;
			break;
		case UPET_ATKRANGE:
			pd->status.rhw.range = (uint16)value;
			break;
		case UPET_ATKMIN:
			pd->status.rhw.atk = (uint16)value;
			break;
		case UPET_ATKMAX:
			pd->status.rhw.atk2 = (uint16)value;
			break;
		case UPET_MATKMIN:
			pd->status.matk_min = (uint16)value;
			break;
		case UPET_MATKMAX:
			pd->status.matk_max = (uint16)value;
			break;
		case UPET_DEF:
			pd->status.def = (defType)value;
			break;
		case UPET_MDEF:
			pd->status.mdef = (defType)value;
			break;
		case UPET_HIT:
			pd->status.hit = (int16)value;
			break;
		case UPET_FLEE:
			pd->status.flee = (int16)value;
			break;
		case UPET_PDODGE:
			pd->status.flee2 = (int16)value;
			break;
		case UPET_CRIT:
			pd->status.cri = (int16)value;
			break;
		case UPET_RACE:
			pd->status.race = (unsigned char)value;
			break;
		case UPET_ELETYPE:
			pd->status.def_ele = (unsigned char)value;
			break;
		case UPET_ELELEVEL:
			pd->status.ele_lv = (unsigned char)value;
			break;
		case UPET_AMOTION:
			pd->status.amotion = (int16)value;
			break;
		case UPET_ADELAY:
			pd->status.adelay = (int16)value;
			break;
		case UPET_DMOTION:
			pd->status.dmotion = (int16)value;
			break;
		case UPET_GROUP_ID:
	...
	case BL_MER:
	{
		...
		{
		case UMER_SIZE:
			mc->battle_status.size = mc->base_status.size = (unsigned char)value;
			break;
		case UMER_HP:
		...
		case UMER_MAXHP:
			...
			break;
		case UMER_MASTERCID:
			mc->mercenary.char_id = (uint32)value;
			break;
		case UMER_MAPID:
			if (mapname)
				value = map_mapname2mapid(mapname);
		...
		case UMER_Y:
			...
			break;
		case UMER_KILLCOUNT:
			mc->mercenary.kill_count = (uint32)value;
			break;
		case UMER_LIFETIME:
			mc->mercenary.life_time = (uint32)value;
			break;
		case UMER_SPEED:
		...
		case UMER_LOOKDIR:
			unit_setdir(bl, (uint8)value);
			break;
		case UMER_CANMOVETICK:
			mc->ud.canmove_tick = value > 0 ? (uint32)value : 0;
			break;
		case UMER_STR:
		...
		case UMER_CRIT:
			...
			break;
		case UMER_RACE:
			mc->battle_status.race = mc->base_status.race = (unsigned char)value;
			break;
		case UMER_ELETYPE:
	...
	case BL_ELEM:
	{
		...
		{
		case UELE_SIZE:
			ed->battle_status.size = ed->base_status.size = (unsigned char)value;
			break;
		case UELE_HP:
		...
		case UELE_MAXSP:
			...
			break;
		case UELE_MASTERCID:
			ed->elemental.char_id = (uint32)value;
			break;
		case UELE_MAPID:
			if (mapname)
				value = map_mapname2mapid(mapname);
		...
		case UELE_Y:
			...
			break;
		case UELE_LIFETIME:
			ed->elemental.life_time = (uint32)value;
			break;
		case UELE_MODE:
		...
		case UELE_LOOKDIR:
			unit_setdir(bl, (uint8)value);
			break;
		case UELE_CANMOVETICK:
			ed->ud.canmove_tick = value > 0 ? (uint32)value : 0;
			break;
		case UELE_STR:
		...
		case UELE_CRIT:
			...
			break;
		case UELE_RACE:
			ed->battle_status.race = ed->base_status.race = (unsigned char)value;
			break;
		case UELE_ELETYPE:
	...
	case BL_NPC:
	{
		...
		case UNPC_LUK:
			...
			status_calc_misc(bl, &nd->status, nd->level);
			break;
		case UNPC_PLUSALLSTAT:
			nd->stat_point = (uint32)value;
			break;
		case UNPC_ATKRANGE:
			nd->status.rhw.range = (uint16)value;
			break;
		case UNPC_ATKMIN:
			nd->status.rhw.atk = (uint16)value;
			break;
		case UNPC_ATKMAX:
			nd->status.rhw.atk2 = (uint16)value;
			break;
		case UNPC_MATKMIN:
			nd->status.matk_min = (uint16)value;
			break;
		case UNPC_MATKMAX:
			nd->status.matk_max = (uint16)value;
			break;
		case UNPC_DEF:
			nd->status.def = (defType)value;
			break;
		case UNPC_MDEF:
			nd->status.mdef = (defType)value;
			break;
		case UNPC_HIT:
			nd->status.hit = (int16)value;
			break;
		case UNPC_FLEE:
			nd->status.flee = (int16)value;
			break;
		case UNPC_PDODGE:
			nd->status.flee2 = (int16)value;
			break;
		case UNPC_CRIT:
			nd->status.cri = (int16)value;
			break;
		case UNPC_RACE:
			nd->status.race = (unsigned char)value;
			break;
		case UNPC_ELETYPE:
			nd->status.def_ele = (unsigned char)value;
			break;
		case UNPC_ELELEVEL:
			nd->status.ele_lv = (unsigned char)value;
			break;
		case UNPC_AMOTION:
			nd->status.amotion = (int16)value;
			break;
		case UNPC_ADELAY:
			nd->status.adelay = (int16)value;
			break;
		case UNPC_DMOTION:
			nd->status.dmotion = (int16)value;
			break;
		case UNPC_SEX:
		...
		case UNPC_HAIRCOLOR:
			clif_changelook(bl, LOOK_HAIR_COLOR, (uint16)value);
			break;
		case UNPC_HEADBOTTOM:
			clif_changelook(bl, LOOK_HEAD_BOTTOM, (uint16)value);
			break;
		case UNPC_HEADMIDDLE:
			clif_changelook(bl, LOOK_HEAD_MID, (uint16)value);
			break;
		case UNPC_HEADTOP:
			clif_changelook(bl, LOOK_HEAD_TOP, (uint16)value);
			break;
		case UNPC_CLOTHCOLOR:
			clif_changelook(bl, LOOK_CLOTHES_COLOR, (uint16)value);
			break;
		case UNPC_SHIELD:
			clif_changelook(bl, LOOK_SHIELD, (uint16)value);
...
BUILDIN_FUNC(waitingroom2bg)
{
	...
	for (i = 0; i < cd->users; i++)
	{ // Only add those who are in the chat room
		map_session_data *sd;
...
/**
 * Atualiza o placar de jogadores vivos da arena 7x7
 * arena7x7_updatescore(<match_id>);
 */
BUILDIN_FUNC(arena7x7_updatescore)
{
	...
	if (!match)
	{
		ShowWarning("buildin_arena7x7_updatescore: Partida %u nao encontrada\n", match_id);
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
...
/**
 * Obt�m a contagem de jogadores vivos em um time
 * arena7x7_getalive(<match_id>, <team>);
 * team: 1 = azul, 2 = vermelho
 */
BUILDIN_FUNC(arena7x7_getalive)
{
	uint32 match_id = script_getnum(st, 2);
	...
	if (team == 1)
	{
		script_pushint(st, match->blue_alive);
	}
	else if (team == 2)
	{
		script_pushint(st, match->red_alive);
	}
	else
	{
		script_pushint(st, 0);
...
BUILDIN_FUNC(arena7x7_isplayerdead)
{
	...
	script_pushint(st, arena7x7_is_player_dead(char_id) ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}

/**
 * Restaura todos os jogadores mortos de uma partida
 * arena7x7_restoreall(<match_id>);
 */
BUILDIN_FUNC(arena7x7_restoreall)
{
	uint32 match_id = script_getnum(st, 2);
	auto match = arena7x7_get_match(match_id);
	...
	for (auto &kv : match->players)
	{
		...
		{
			map_session_data *sd = map_charid2sd(stats->char_id);
			if (sd)
			{
...
/**
 * Retorna o nome da guild de um time na partida
 * arena7x7_getguildname(<match_id>, <team>);
 * team: 1 = Azul, 2 = Vermelho
 */
BUILDIN_FUNC(arena7x7_getguildname)
{
	uint32 match_id = script_getnum(st, 2);
	int team = script_getnum(st, 3);
	...
	if (team == 1)
	{
		script_pushstrcopy(st, match->blue_guild_name.c_str());
	}
	else if (team == 2)
	{
		script_pushstrcopy(st, match->red_guild_name.c_str());
	}
	else
	{
		script_pushconststr(st, "");
...
static int32 buildin_mobuseskill_sub(struct block_list *bl, va_list ap)
{
	...
	case 2:
		...
		break;
	default:
		tbl = battle_getenemy(md, DEFAULT_ENEMY_TYPE(md), skill_get_range2(md, skill_id, skill_lv, true));
		break;
	}
...
BUILDIN_FUNC(progressbar_npc)
{
	...
	if (!nd->progressbar.timeout)
	{
		...
		if (sd)
		{ // Player attached - keep them from doing other things
			sd->state.workinprogress = WIP_DISABLE_ALL;
	...
	else
	{
		// Continue the script
		if (sd)
		{ // Player attached - remove restrictions
...
BUILDIN_FUNC(npcskill)
{
	...
	if (nd == nullptr)
	{ // ain't possible, but I don't trust people.
		return SCRIPT_CMD_FAILURE;
	}
...
BUILDIN_FUNC(party_destroy)
{
	...
	ARR_FIND(0, MAX_PARTY, i, party->party.member[i].leader);
	if (i >= MAX_PARTY || !party->data[i].sd)
	{ // leader not online
		int32 j;
...
BUILDIN_FUNC(channel_kick)
{
	...
	{
	case CHAN_TYPE_ALLY:
		res = channel_pcquit(tsd, 3);
		break;
	case CHAN_TYPE_MAP:
		res = channel_pcquit(tsd, 4);
		break;
	default:
		res = channel_clean(ch, tsd, 0);
		break;
	}
...
struct script_function buildin_func[] = {
	...
	// fim original
	// in�cio custom pra fazer plagiadora anti mamaco
	BUILDIN_DEF(plagiarizeskill, "ii"),
	BUILDIN_DEF(plagiarizeskillreset, "i"),
	BUILDIN_DEF(copyskill_plagiarism, "vi"),
	BUILDIN_DEF(copyskill_reproduce, "vi"),
	BUILDIN_DEF(removeskill_plagiarism, ""),
	BUILDIN_DEF(removeskill_reproduce, ""),
	BUILDIN_DEF(skill, "vi?"),
	// fim do custom
	...
	// Arena 7x7
	BUILDIN_DEF(arena7x7_updatescore, "i"),
	BUILDIN_DEF(arena7x7_getalive, "ii"),
