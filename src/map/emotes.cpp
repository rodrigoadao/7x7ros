...
// }
//custom pra geral ter emoticon custom
void emotes_get_player_packs(map_session_data *sd)
{
    nullpo_retv(sd);
    ...
    // Custom: Send ALL packs as owned without expiration
    // Iterate through every pack in the database and add them all
    for (auto &k : cash_emotes_db)
    {
        // Add every pack as permanently owned (no expiration)
        packs.push_back({k.first, false, 0});
    }
